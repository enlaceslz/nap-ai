...
export default function LandingPage() {
 ...
 {isConfigOpen && (
 <div className="bg-card/95 backdrop-blur-md p-2 rounded-2xl border border-border shadow-2xl flex items-center gap-1.5 animate-in slide-in-from-bottom-4">
 <button 
 onClick={() => { setActiveTemplate(1); setIsConfigOpen(false); }}
 className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activeTemplate === 1 ? 'bg-blue-600 text-white shadow-md' : 'text-muted-foreground hover:text-white hover:bg-accent'}`}
 >
 1. Tech Dark
 </button>
 <button 
 onClick={() => { setActiveTemplate(2); setIsConfigOpen(false); }}
 className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activeTemplate === 2 ? 'bg-purple-600 text-white shadow-md' : 'text-muted-foreground hover:text-white hover:bg-accent'}`}
 >
 2. Gamer Vibrant
 </button>
 <button 
 onClick={() => { setActiveTemplate(3); setIsConfigOpen(false); }}
 className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all ${activeTemplate === 3 ? 'bg-emerald-600 text-white shadow-md' : 'text-muted-foreground hover:text-white hover:bg-accent'}`}
 >
 3. Clean Family
 </button>
 <Link
 ...
 <div className="bg-background/90 backdrop-blur-xl border border-border shadow-2xl rounded-full p-1.5 flex items-center gap-1.5">
 {mostrarPortal && (
 <Link
 to="/portal"
 ...
 <Link
 to="/admin"
 className="text-muted-foreground hover:text-foreground hover:bg-accent px-3.5 py-2 rounded-full font-bold text-xs sm:text-sm flex items-center gap-1.5 transition-all"
 ...
 className="text-muted-foreground hover:text-foreground hover:bg-accent px-3 py-2 rounded-full font-bold text-xs flex items-center gap-1.5 transition-all"
