...
export default function Campanhas() {
 ...
 {/* Header */}
 <header className="p-6 border-b border-border bg-card/80 backdrop-blur-md flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0 z-10">
 <div>
 ...
 <p className="text-sm text-muted-foreground mt-1">Disparo em massa, discador automático (Asterisk 20+) e réguas de relacionamento.</p>
 {toastMsg && (
 ...
 <button onClick={() => setFeedbackCampanha(null)} className="text-muted-foreground hover:text-foreground">
 <X size={14} />
 </button>
 </div>
 )}
 ...
 {/* Tabs */}
 <div className="px-4 sm:px-6 pt-6 flex gap-4 border-b border-border shrink-0 overflow-x-auto whitespace-nowrap" style={{ scrollbarWidth: 'none' }}>
 <button 
 onClick={() => setActiveTab('regua')}
 className={`flex items-center gap-2 pb-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'regua' ? 'border-amber-500 text-amber-400' : 'border-transparent text-muted-foreground hover:text-muted-foreground'}`}
 >
 <Zap size={18} className="text-amber-400" /> Régua de Cobrança IA (Auto-Billing)
 </button>
 <button 
 onClick={() => setActiveTab('whatsapp')}
 className={`flex items-center gap-2 pb-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'whatsapp' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-muted-foreground hover:text-muted-foreground'}`}
 >
 <MessageCircle size={18} /> Disparo de WhatsApp
 </button>
 <button 
 onClick={() => setActiveTab('voz')}
 className={`flex items-center gap-2 pb-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'voz' ? 'border-blue-600 text-blue-400' : 'border-transparent text-muted-foreground hover:text-muted-foreground'}`}
 >
 <PhoneOutgoing size={18} /> Discador Automático (Voz)
 </button>
 <button 
 onClick={() => setActiveTab('push')}
 className={`flex items-center gap-2 pb-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'push' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-muted-foreground hover:text-muted-foreground'}`}
 >
 <Bell size={18} /> Notificações Push (PWA)
 </button>
 <button 
 onClick={() => setActiveTab('ativos')}
 className={`flex items-center gap-2 pb-4 text-sm font-bold border-b-2 transition-colors ${activeTab === 'ativos' ? 'border-fuchsia-500 text-fuchsia-400' : 'border-transparent text-muted-foreground hover:text-muted-foreground'}`}
 >
 ...
 <div className="bg-card border border-blue-500/20 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs bg-gradient-to-r from-blue-950/20 via-slate-900 to-slate-900">
 ...
 <div>
 <h3 className="text-lg font-bold text-foreground font-outfit flex flex-wrap items-center gap-2">
 ...
 className="px-3 py-2 bg-white/5 hover:bg-accent text-muted-foreground hover:text-foreground rounded-xl text-xs font-bold transition-all border border-border flex items-center gap-1.5"
 >
 ...
 className="p-2 bg-white/5 hover:bg-accent rounded-xl text-muted-foreground hover:text-foreground transition-colors border border-border"
 ...
 <button onClick={() => setFeedbackRegua(null)} className="text-muted-foreground hover:text-foreground">
 <X size={14} />
 </button>
 </div>
 )}
 ...
 {/* Fase 1: D-3 */}
 <div onClick={() => showToast('Abrindo detalhes do card')} className="bg-background border border-border hover:border-blue-500/30 rounded-2xl p-5 flex flex-col justify-between transition-all group cursor-pointer">
 <div>
 ...
 <h4 className="font-bold text-foreground text-sm">Lembrete Amigável</h4>
 ...
 <div className="mt-3 p-3 bg-white/[0.02] border border-border rounded-xl text-[11px] text-muted-foreground font-mono line-clamp-2">
 ...
 <h4 className="font-bold text-foreground text-sm">Vencimento da Fatura</h4>
 <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
 ...
 <h4 className="font-bold text-foreground text-sm">Auto-Desbloqueio & PIX</h4>
 <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
 ...
 <h4 className="font-bold text-foreground text-sm">Aviso de Suspensão</h4>
 <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
 ...
 className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${filtroFaseFila === 'todas' ? 'bg-blue-600 text-white' : 'text-muted-foreground hover:text-white'}`}
 >
 Todas
 </button>
 <button 
 onClick={() => setFiltroFaseFila('d_menos_3')}
 className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${filtroFaseFila === 'd_menos_3' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'text-muted-foreground hover:text-white'}`}
 >
 D-3
 </button>
 <button 
 onClick={() => setFiltroFaseFila('d_zero')}
 className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${filtroFaseFila === 'd_zero' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'text-muted-foreground hover:text-white'}`}
 >
 D0
 </button>
 <button 
 onClick={() => setFiltroFaseFila('d_mais_3')}
 className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${filtroFaseFila === 'd_mais_3' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'text-muted-foreground hover:text-white'}`}
 >
 D+3
 </button>
 <button 
 onClick={() => setFiltroFaseFila('d_mais_7')}
 className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${filtroFaseFila === 'd_mais_7' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'text-muted-foreground hover:text-white'}`}
 >
 D+7
 </button>
 </div>
 ...
 className="bg-card border border-border rounded-xl px-3 py-1.5 text-xs text-muted-foreground outline-none"
 >
 ...
 <thead className="text-muted-foreground uppercase tracking-wider border-b border-border text-[10px]">
 <tr>
 ...
 <div className="font-bold text-foreground">{cli.nome}</div>
 <div className="text-[11px] text-muted-foreground flex items-center gap-1.5 mt-0.5">
 <span>{cli.telefone}</span>
 <span>•</span>
 <span className="text-muted-foreground">{cli.bairro}</span>
 </div>
 </td>
 <td className="py-3 px-3">
 <div className="text-foreground font-medium">{cli.plano}</div>
 ...
 <div className="text-[11px] text-muted-foreground mt-1">{cli.vencimento}</div>
 </td>
 <td className="py-3 px-3">
 ...
 <div className="text-[10px] text-muted-foreground mt-0.5">{cli.ultimoEnvio || 'Hoje'}</div>
 </div>
 ) : (
 ...
 className="p-1.5 bg-white/5 hover:bg-accent rounded-lg text-muted-foreground hover:text-foreground transition-all border border-border"
 ...
 <div>
 <h2 className="text-xl font-bold text-foreground font-outfit">Templates e Ativos</h2>
 <p className="text-sm text-muted-foreground mt-0.5">Gestão de HSM (WhatsApp) e Áudios de URA</p>
 </div>
 </div>
 ...
 <div onClick={() => showToast('Editando template HSM')} className="bg-background p-5 rounded-2xl border border-border flex flex-col justify-between hover:border-fuchsia-500/30 transition-all group cursor-pointer">
 <div>
 ...
 <h3 className="text-sm font-bold text-foreground mb-2">cobranca_aviso_d3</h3>
 <p className="text-xs text-muted-foreground italic">"Olá {`{{1}}`}! Identificamos que sua fatura vence em {`{{2}}`}. O código PIX Copia e Cola é: {`{{3}}`}..."</p>
 </div>
 <div className="mt-4 pt-4 border-t border-border flex gap-2">
 <button onClick={(e) => { e.stopPropagation(); showToast('Visualizando ativo...'); }} className="flex-1 bg-white/5 hover:bg-accent text-foreground text-xs py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5"><Eye size={12} /> Ver</button>
 <button onClick={(e) => { e.stopPropagation(); showToast('Ativo copiado para a área de transferência!'); }} className="flex-1 bg-white/5 hover:bg-accent text-foreground text-xs py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5"><Copy size={12} /> Copiar</button>
 </div>
 </div>
 ...
 <h3 className="text-sm font-bold text-foreground mb-2">oferta_wifi6_mesh</h3>
 <p className="text-xs text-muted-foreground italic">"Oi {`{{1}}`}! Como assinante fiel, liberamos um upgrade para roteador Wi-Fi 6 Mesh na sua casa por apenas {`{{2}}`}..."</p>
 </div>
 <div className="mt-4 pt-4 border-t border-border flex gap-2">
 <button onClick={(e) => { e.stopPropagation(); showToast('Visualizando ativo...'); }} className="flex-1 bg-white/5 hover:bg-accent text-foreground text-xs py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5"><Eye size={12} /> Ver</button>
 <button onClick={(e) => { e.stopPropagation(); showToast('Ativo copiado para a área de transferência!'); }} className="flex-1 bg-white/5 hover:bg-accent text-foreground text-xs py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5"><Copy size={12} /> Copiar</button>
 ...
 <h3 className="text-sm font-bold text-foreground mb-2">ura_aviso_corte.mp3</h3>
 <p className="text-xs text-muted-foreground italic flex items-center gap-1"><Clock size={12} /> Duração: 00:15</p>
 </div>
 <div className="mt-4 pt-4 border-t border-border flex gap-2">
 <button onClick={(e) => { e.stopPropagation(); showToast('Reproduzindo áudio...'); }} className="flex-1 bg-white/5 hover:bg-accent text-foreground text-xs py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5"><Play size={12} /> Ouvir</button>
 <button onClick={(e) => { e.stopPropagation(); showToast('Trocar arquivo de áudio...'); }} className="flex-1 bg-white/5 hover:bg-accent text-foreground text-xs py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5"><RefreshCw size={12} /> Trocar</button>
 </div>
 </div>
 </div>
 </div>
 ...
 <div>
 <h3 className="font-bold text-foreground font-outfit">Transmitir Alerta Web Push (PWA)</h3>
 <p className="text-xs text-muted-foreground">Envia notificação instantânea para a tela dos clientes com PWA instalado.</p>
 </div>
 </div>
 ...
 <label className="block text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5">Título da Notificação</label>
 <input 
 type="text" 
 ...
 className="w-full bg-background border border-border rounded-xl px-4 py-2.5 text-sm text-foreground outline-none focus:border-indigo-600 "
 required
 />
 </div>
 <div>
 <label className="block text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5">Pilar / Categoria</label>
 <select 
 ...
 <div>
 <label className="block text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5">Mensagem / Texto do Push</label>
 <textarea 
 ...
 className="w-full bg-background border border-border rounded-xl p-3 text-sm text-foreground outline-none focus:border-indigo-600 "
 required
 />
 </div>
 ...
 <div className="bg-card rounded-3xl border border-border p-6 ">
 <h4 className="font-bold text-foreground font-outfit mb-4 flex items-center gap-2">
 <Smartphone size={18} className="text-muted-foreground" /> Dispositivos Inscritos
 </h4>
 <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
 {(pushStatus?.inscricoes || []).map((sub: any, idx: number) => (
 <div key={idx} className="p-3 bg-background rounded-2xl border border-border flex items-center justify-between">
 <div>
 <p className="text-sm font-bold text-foreground">{sub.cliente_nome}</p>
 <p className="text-xs text-muted-foreground">{sub.dispositivo} • ID: {sub.cliente_id}</p>
 </div>
 ...
 <div className="bg-card rounded-3xl border border-border p-6 ">
 <h4 className="font-bold text-foreground font-outfit mb-4 flex items-center gap-2">
 <Bell size={18} className="text-muted-foreground" /> Histórico de Envios
 </h4>
 <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
 {(pushStatus?.historico_recente || []).map((item: any, idx: number) => (
 <div key={idx} className="p-3 bg-background rounded-2xl border border-border">
 <div className="flex items-center justify-between mb-1">
 <span className="text-xs font-bold text-foreground">{item.titulo}</span>
 <span className="text-[10px] text-muted-foreground">{item.enviado_em}</span>
 </div>
 ...
 className="w-full bg-background border border-border rounded-lg pl-9 pr-4 py-2 text-sm text-foreground outline-none focus:border-blue-600/50 "
 />
 </div>
 </div>
 ...
 <th className="px-6 py-4 text-[10px] uppercase tracking-wider font-bold text-muted-foreground">Campanha</th>
 <th className="px-6 py-4 text-[10px] uppercase tracking-wider font-bold text-muted-foreground">Progresso</th>
 <th className="px-6 py-4 text-[10px] uppercase tracking-wider font-bold text-muted-foreground">Métricas</th>
 <th className="px-6 py-4 text-[10px] uppercase tracking-wider font-bold text-muted-foreground">Status</th>
 <th className="px-6 py-4 text-center text-[10px] uppercase tracking-wider font-bold text-muted-foreground">Ações</th>
 </tr>
 </thead>
 ...
 <p className="font-bold text-foreground text-sm group-hover:text-blue-400 transition-colors">{camp.nome}</p>
 <p className="text-xs text-muted-foreground font-mono mt-0.5">ID: CMP-{(1000 + camp.id).toString()}</p>
 </td>
 <td className="px-6 py-4">
 <div className="flex justify-between text-xs mb-1">
 <span className="text-muted-foreground">{camp.processados} / {camp.leads}</span>
 <span className="font-bold text-muted-foreground">{Math.round(percent)}%</span>
 </div>
 ...
 <button className="w-8 h-8 bg-white/5 border border-border text-muted-foreground hover:bg-accent hover:text-foreground rounded-lg flex items-center justify-center transition-all mx-auto">
 <BarChart2 size={14} />
 </button>
 )}
 </td>
 ...
 <div>
 <h3 className="font-bold text-foreground text-base font-outfit">Criar Nova Campanha</h3>
 <p className="text-xs text-muted-foreground">Disparo em lote via WhatsApp WABA ou Discador Asterisk</p>
 </div>
 </div>
 <button 
 ...
 className="w-full bg-background border border-border rounded-xl px-4 py-2.5 text-sm text-foreground outline-none focus:border-blue-500 transition-colors"
 required
 />
 </div>
 ...
 className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-xs text-foreground outline-none focus:border-blue-500 transition-colors"
 >
 ...
 className="w-full bg-background border border-border rounded-xl p-3 text-xs text-foreground outline-none focus:border-blue-500 transition-colors resize-none"
 />
 </div>
 ...
 type="button"
 onClick={() => setIsModalNovaCampanhaOpen(false)}
 className="px-4 py-2 text-xs font-bold text-muted-foreground hover:text-foreground rounded-xl hover:bg-accent transition-colors"
 >
 Cancelar
 </button>
 <button
 ...
 <div className="bg-card border border-border rounded-3xl w-full max-w-xl p-6 shadow-2xl overflow-hidden">
 ...
 <div>
 <h3 className="font-bold text-foreground text-base">Parâmetros da Régua de Cobrança</h3>
 <p className="text-xs text-muted-foreground">Configure horários de disparo, canais ativos e regras financeiras</p>
 </div>
 </div>
 <button 
 ...
 {/* Status da Régua */}
 <div className="p-3.5 bg-white/[0.02] border border-border rounded-2xl flex items-center justify-between">
 <div>
 <span className="text-xs font-bold text-foreground block">Automação da Régua Ativa</span>
 <span className="text-[11px] text-muted-foreground">Permitir que o sistema analise e dispare cobranças nos horários definidos</span>
 </div>
 <input 
 type="checkbox"
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <span className="text-[11px] text-muted-foreground block mb-1">Início dos Disparos</span>
 <input 
 type="time"
 ...
 <div>
 <span className="text-[11px] text-muted-foreground block mb-1">Término Máximo</span>
 <input 
 type="time"
 ...
 <label className="flex items-center gap-2.5 p-2.5 bg-white/[0.02] border border-border rounded-xl cursor-pointer hover:bg-white/[0.04]">
 <input 
 type="checkbox"
 ...
 />
 <span className="text-xs text-muted-foreground font-medium">WhatsApp WABA (Oficial)</span>
 </label>
 ...
 />
 <span className="text-xs text-muted-foreground font-medium">SMS Flash / Gateway</span>
 </label>
 ...
 />
 <span className="text-xs text-muted-foreground font-medium">Push Notification (PWA)</span>
 </label>
 ...
 />
 <span className="text-xs text-muted-foreground font-medium">E-mail Transacional</span>
 </label>
 </div>
 </div>
 ...
 {modalTemplatesRegua && (
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
 <div className="bg-card border border-border rounded-3xl w-full max-w-4xl p-6 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
 ...
 <div>
 <h3 className="font-bold text-foreground text-base">Templates de Notificação da Régua</h3>
 <p className="text-xs text-muted-foreground">Edite as mensagens de cada fase e teste o envio em tempo real</p>
 </div>
 </div>
 <button 
 ...
 {/* Abas das 4 Fases */}
 <div className="flex items-center gap-2 mt-4 pb-3 border-b border-border shrink-0 overflow-x-auto">
 <button
 onClick={() => { setTemplateFaseAtiva('d_menos_3'); setResultadoTeste(null); }}
 className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 ${templateFaseAtiva === 'd_menos_3' ? 'bg-blue-600 text-white' : 'bg-white/5 text-muted-foreground hover:text-white'}`}
 >
 Fase 1 • D-3 (Preventivo)
 </button>
 <button
 onClick={() => { setTemplateFaseAtiva('d_zero'); setResultadoTeste(null); }}
 className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 ${templateFaseAtiva === 'd_zero' ? 'bg-amber-600 text-white' : 'bg-white/5 text-muted-foreground hover:text-white'}`}
 >
 Fase 2 • D0 (Vence Hoje)
 </button>
 <button
 onClick={() => { setTemplateFaseAtiva('d_mais_3'); setResultadoTeste(null); }}
 className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 ${templateFaseAtiva === 'd_mais_3' ? 'bg-emerald-600 text-white' : 'bg-white/5 text-muted-foreground hover:text-white'}`}
 >
 Fase 3 • D+3 (Tolerância 24h)
 </button>
 <button
 onClick={() => { setTemplateFaseAtiva('d_mais_7'); setResultadoTeste(null); }}
 className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 ${templateFaseAtiva === 'd_mais_7' ? 'bg-red-600 text-white' : 'bg-white/5 text-muted-foreground hover:text-white'}`}
 >
 ...
 className="w-full bg-background border border-border rounded-2xl p-3.5 text-xs text-foreground outline-none focus:border-blue-500 transition-colors font-mono leading-relaxed resize-none"
 ...
 {/* Tags Disponíveis */}
 <div>
 <span className="text-[11px] text-muted-foreground font-bold block mb-1.5">Tags Dinâmicas Disponíveis (Clique para inserir):</span>
 ...
 className="px-2 py-1 bg-white/5 hover:bg-blue-500/20 text-muted-foreground hover:text-blue-300 rounded-lg text-[10px] font-mono border border-border transition-all"
 >
 {tag}
 </button>
 ))}
 ...
 <div className="bg-[#0b141a] border border-border rounded-2xl p-4 flex flex-col justify-between shadow-inner min-h-[190px]">
 ...
 <form onSubmit={handleSimularTesteEnvio} className="bg-background border border-border rounded-2xl p-3.5 space-y-2.5">
 <div className="flex items-center justify-between">
 <span className="text-xs font-bold text-foreground flex items-center gap-1.5">
 <Smartphone size={14} className="text-emerald-400" />
 Testar Envio com Número Real
 </span>
 <span className="text-[10px] text-muted-foreground font-mono">Simulador WABA</span>
 </div>
 ...
 className="w-full bg-card border border-border rounded-xl px-3 py-2 text-xs text-foreground outline-none focus:border-emerald-500"
 />
 <button
 type="submit"
 ...
 <div className="pt-4 mt-4 border-t border-border flex items-center justify-end gap-3 shrink-0">
 <button
 type="button"
 onClick={() => setModalTemplatesRegua(false)}
 className="px-4 py-2 text-xs font-bold text-muted-foreground hover:text-foreground rounded-xl hover:bg-accent transition-colors"
