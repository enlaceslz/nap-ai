  ...
  const [historyData] = useState([
    {
      ...
      mensagens: [
        ...
        { remetente: 'ia', texto: 'Bom dia! Sou a IA do NAP. Pode me informar o CPF do titular?', hora: '09:15' },
    ...
    <div className="p-6 max-w-[1600px] mx-auto h-[calc(100vh-64px)] flex flex-col relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-3">
          <button className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-slate-200 dark:hover:bg-slate-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 shrink-0">
        ...
        >
          ...
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Total no Mês</p>
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white leading-none mt-1">{metrics.total}</h3>
          </div>
        </button>
        ...
        >
          ...
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Resolvidos (FCR)</p>
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white leading-none mt-1">{metrics.resolvidos}</h3>
          </div>
        </button>
        ...
        <button 
          ...
          className={`text-left p-5 rounded-2xl border flex items-center gap-4 transition-all ${
            kpiFilter === 'pendentes' 
              ? 'bg-slate-50 dark:bg-slate-800/80 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.1)]' 
              : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50'
          }`}
        >
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
          ...
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Pendentes / OS</p>
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white leading-none mt-1">{metrics.pendentes}</h3>
          </div>
        </button>
        ...
        <button 
          ...
          className={`text-left p-5 rounded-2xl border flex items-center gap-4 transition-all ${
            kpiFilter === 'ia' 
              ? 'bg-slate-50 dark:bg-slate-800/80 border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.1)]' 
              : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50'
          }`}
        >
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
          ...
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Atendidos por IA</p>
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white leading-none mt-1">{metrics.ia}</h3>
          </div>
        </button>
      </div>

      {/* Filters Bar */}
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-4 mb-6 shrink-0">
        <div className="flex-1 relative">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-sm text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
          />
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="relative w-full sm:w-auto">
            <select 
              ...
              className="w-full sm:w-auto appearance-none pl-10 pr-10 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-sm text-slate-700 dark:text-slate-300 focus:outline-none focus:border-blue-500"
            >
      ...
      <div className="flex-1 overflow-hidden bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800 text-xs uppercase font-bold text-slate-500 dark:text-slate-400">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {filteredData.map(chat => (
                <tr key={chat.id} onClick={() => setSelectedChat(chat)} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors cursor-pointer group">
                  <td className="p-4">
                    <span className="font-mono font-bold text-blue-600 dark:text-blue-400">{chat.protocolo}</span>
                    <div className="text-[11px] text-slate-400 mt-0.5">{format(new Date(chat.inicio), 'dd/MM/yyyy HH:mm')}</div>
                  </td>
                  <td className="p-4">
                    <div className="font-bold text-slate-900 dark:text-white">{chat.cliente}</div>
                  ...
                  <td className="p-4 hidden sm:table-cell">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{chat.operador}</span>
                    </div>
                  </td>
                  ...
                  <td className="p-4 text-center">
                    <button 
                      ...
                      className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-blue-500 rounded-lg transition-colors"
      ...
      {selectedChat && (
        <>
          {/* Overlay */}
          <div 
            className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-40 transition-opacity animate-in fade-in"
          ...
          <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-slate-900 shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0">
              <div>
                ...
                <p className="font-mono text-sm text-slate-500 mt-1">{selectedChat.protocolo}</p>
              </div>
              <button 
                onClick={() => setSelectedChat(null)}
            ...
            {/* Customer KPIs */}
            <div className="bg-slate-50 dark:bg-slate-950 p-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2 text-slate-900 dark:text-white font-bold">
                  <User size={16} className="text-blue-500" />
                  {selectedChat.cliente}
                </div>
                <div className="flex items-center gap-1.5 text-xs text-slate-500 bg-white dark:bg-slate-900 px-2 py-1 rounded-md border border-slate-200 dark:border-slate-800">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                  <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Motivo (Tabulação)</p>
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300 line-clamp-1">{selectedChat.tabulacao}</p>
                </div>
                <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-800">
                  <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Conclusão</p>
                ...
                if (isSystem) {
                  return (
                    <div key={idx} className="flex justify-center">
                      <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-2">
                        <AlertCircle size={12} />
                        {msg.texto}
                      </span>
                    </div>
