    ...
    <div className="flex-1 flex flex-col h-full bg-slate-950 text-slate-300 overflow-hidden font-sans relative">
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 mt-0.5">Gestão de Almoxarifado, CPEs e Rastreamento de Veículos</p>
            </div>
          </div>
        </div>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-slate-900 border border-white/5 rounded-2xl p-5 shadow-lg">
                  ...
                  <p className="text-3xl font-bold text-white font-outfit mb-1">{estoque.length}</p>
                  <p className="text-xs text-slate-400 uppercase tracking-wider font-bold">Total de Itens (SKUs)</p>
                </div>
                ...
                <div className="bg-slate-900 border border-white/5 rounded-2xl p-5 shadow-lg">
                  ...
                  <p className="text-3xl font-bold text-white font-outfit mb-1">12</p>
              ...
              <div className="bg-slate-900 border border-white/5 rounded-3xl overflow-hidden shadow-xl">
                <div className="px-6 py-5 border-b border-white/5 flex flex-wrap justify-between items-center gap-4 bg-slate-900/50">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="pl-10 pr-4 py-2 bg-slate-950 border border-white/10 rounded-xl text-sm text-white focus:ring-2 focus:ring-blue-500 outline-none w-64"
                      />
                    </div>
                    <button 
                    ...
                    <tbody className="divide-y divide-white/5 bg-slate-900">
                      {filteredEstoque.map(item => (
                        <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                          ...
                          <td className="px-6 py-4 font-bold text-white">{item.nome}</td>
                          <td className="px-6 py-4">
                            <span className="bg-slate-800 text-slate-300 px-2 py-1 rounded-md text-[10px] font-medium border border-white/5">
                              {item.categoria}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <span className={`font-mono font-bold text-base ${item.status === 'critico' ? 'text-red-400' : item.status === 'alerta' ? 'text-amber-400' : 'text-slate-200'}`}>
                              {item.qtd}
                            </span>
                            <span className="text-[10px] text-slate-500 block">Mín: {item.minimo}</span>
                          </td>
                          ...
                          <td className="px-6 py-4 text-right">
                            <button 
                              ...
                              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold rounded-lg transition-colors border border-white/5"
                            >
                              Transferir
                            </button>
                          </td>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-slate-900 border border-white/5 rounded-2xl p-5 shadow-lg">
                  ...
                  <p className="text-3xl font-bold text-white font-outfit mb-1">{frota.length}</p>
                  <p className="text-xs text-slate-400 uppercase tracking-wider font-bold">Veículos Totais</p>
                </div>
                ...
                <div className="bg-slate-900 border border-white/5 rounded-2xl p-5 shadow-lg">
                  ...
                  <p className="text-xs text-slate-400 uppercase tracking-wider font-bold">Pátio / Base</p>
                </div>
                    ...
                    <tbody className="divide-y divide-white/5 bg-slate-900">
                      {filteredFrota.map(v => (
                        <tr key={v.id} className="hover:bg-white/5 transition-colors group">
                          <td className="px-6 py-4">
                            <p className="font-bold text-white text-xs">{v.id}</p>
                            <p className="font-mono text-[10px] text-slate-500 bg-slate-950 px-1 py-0.5 rounded w-fit border border-white/5">{v.placa}</p>
                          </td>
                          <td className="px-6 py-4 font-bold text-slate-200">{v.veiculo}</td>
                          <td className="px-6 py-4 text-slate-300">{v.tecnico}</td>
                          <td className="px-6 py-4 font-mono text-xs">{v.km} km</td>
                          <td className="px-6 py-4 text-center">
                            <div className="flex flex-col items-center">
                              <span className={`text-[10px] font-bold ${v.combustivel < 20 ? 'text-red-400' : 'text-slate-400'}`}>{v.combustivel}%</span>
                              <div className="w-10 h-1.5 bg-slate-800 rounded-full mt-1 overflow-hidden border border-white/5">
      ...
      {showNovaEntrada && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between p-5 border-b border-white/5">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <Plus size={18} className="text-blue-400" /> Nova Entrada de Estoque
              </h3>
              <button onClick={() => setShowNovaEntrada(false)} className="text-slate-400 hover:text-white">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={handleNovaEntradaSubmit} className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Descrição do Item</label>
                <input required name="nome" type="text" className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2 text-white outline-none focus:border-blue-500" placeholder="Ex: Roteador AC1200" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Categoria</label>
                <select required name="categoria" className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2 text-white outline-none focus:border-blue-500">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Qtd Inicial</label>
                  <input required name="qtd" type="number" min="0" defaultValue="1" className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2 text-white outline-none focus:border-blue-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Estoque Mínimo</label>
                  <input required name="minimo" type="number" min="1" defaultValue="10" className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2 text-white outline-none focus:border-blue-500" />
                </div>
              </div>
              <div className="pt-4 flex justify-end gap-3 border-t border-white/5 mt-6">
                <button type="button" onClick={() => setShowNovaEntrada(false)} className="px-4 py-2 text-sm font-bold text-slate-300 hover:text-white">Cancelar</button>
      ...
      {showTransferModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between p-5 border-b border-white/5">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            <form onSubmit={(e) => {
              ...
              <p className="text-sm text-slate-300 mb-4">
                Selecione a quantidade para transferir do <strong className="text-white">Estoque Principal</strong> para uma <strong className="text-white">Viatura/Técnico</strong>.
              </p>
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Quantidade</label>
                <input required name="qtd" type="number" min="1" defaultValue="1" className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2 text-white outline-none focus:border-blue-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Destino (Técnico/Viatura)</label>
                <select required className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2 text-white outline-none focus:border-blue-500">
                  {frota.map(v => (
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-white/5 mt-6">
                <button type="button" onClick={() => setShowTransferModal(null)} className="px-4 py-2 text-sm font-bold text-slate-300 hover:text-white">Cancelar</button>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between p-5 border-b border-white/5">
              ...
              <button onClick={() => setShowFrotaAction(null)} className="text-slate-400 hover:text-white">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={(e) => {
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Status da Viatura</label>
                <select required name="status" defaultValue={frota.find(v => v.id === showFrotaAction)?.status} className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2 text-white outline-none focus:border-blue-500">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Nível de Combustível (%)</label>
                <input required name="combustivel" type="number" min="0" max="100" defaultValue={frota.find(v => v.id === showFrotaAction)?.combustivel} className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-2 text-white outline-none focus:border-blue-500" />
              </div>
              <div className="pt-4 flex justify-end gap-3 border-t border-white/5 mt-6">
                <button type="button" onClick={() => setShowFrotaAction(null)} className="px-4 py-2 text-sm font-bold text-slate-300 hover:text-white">Cancelar</button>
