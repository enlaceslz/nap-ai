...
export default function Template2({ config: propsConfig }: TemplateProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-outfit overflow-x-hidden selection:bg-purple-500/30">
      {/* Header */}
      <header className="fixed top-0 w-full z-50 bg-slate-950/60 backdrop-blur-xl border-b border-purple-500/10">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-6xl">
            {planos.map((plan, i) => (
              <motion.div 
                key={i}
                whileHover={{ y: -10 }}
                className={`bg-slate-900 border border-purple-500/20 rounded-3xl p-8 transition-all ${plan.glow} ${plan.highlight ? 'scale-105 bg-gradient-to-b from-[#1a0b33] to-[#120524]' : ''}`}
              >
                {plan.highlight && (
