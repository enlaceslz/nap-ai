...
export default function Kanban({ type }: { type: "Suporte" | "Vendas" | "Cobranca" }) {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (source.droppableId !== destination.droppableId) {
      ...
      // Persistência nativa no backend do NAP e Automações WABA
      try {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-950 overflow-hidden text-slate-100 font-sans">
      ...
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 bg-slate-900 border border-[#0a50ff]/40 text-white px-4 py-3 rounded-2xl shadow-[0_12px_32px_rgba(10,80,255,0.25)] flex items-center gap-3 animate-in slide-in-from-bottom-5 duration-300">
      ...
      <div className="border-b border-slate-800 bg-slate-900/95 backdrop-blur-md px-5 py-4 shrink-0 space-y-3.5">
        
        {/* Row 1: Pipeline Switcher & Actions */}
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
          
          {/* Pipeline Switcher Tabs (Digify Multi-Pipeline) */}
          <div className="flex items-center gap-1.5 p-1 bg-slate-900 border border-slate-700 rounded-2xl">
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${
                    isActive ? 'bg-white/20 text-white font-mono' : 'bg-white/5 text-slate-400'
                  }`}>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
            <div className="relative flex-1 sm:w-60">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-900 hover:bg-slate-800 border border-slate-700 focus:border-[#0a50ff] focus:ring-1 focus:ring-[#0a50ff] rounded-xl text-slate-100 placeholder:text-slate-500 outline-none transition-all"
              />
              {searchTerm && (
                <button 
            ...
            {/* Filter Chips */}
            <div className="flex items-center gap-1 bg-slate-900 border border-slate-700 p-0.5 rounded-xl text-xs">
              <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-2.5 pt-1">
          <button 
            ...
            className={`text-left rounded-xl px-3.5 py-2.5 flex items-center gap-3 transition-all ${
              kpiFilter === 'funil' 
                ? 'bg-slate-800 border border-[#0a50ff]/50 shadow-[0_0_15px_rgba(10,80,255,0.1)]' 
                : 'bg-slate-900/80 border border-slate-800 hover:bg-slate-800/80'
            }`}
          >
          ...
          <button 
            ...
            className={`text-left rounded-xl px-3.5 py-2.5 flex items-center gap-3 transition-all ${
              kpiFilter === 'ticket' 
                ? 'bg-slate-800 border border-[#18c7a8]/50 shadow-[0_0_15px_rgba(24,199,168,0.1)]' 
                : 'bg-slate-900/80 border border-slate-800 hover:bg-slate-800/80'
            }`}
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
            ...
            <div className="min-w-0">
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Ticket Médio</p>
              <p className="text-sm font-extrabold text-white truncate font-mono">
          ...
          <button 
            ...
            className={`text-left rounded-xl px-3.5 py-2.5 flex items-center gap-3 transition-all ${
              kpiFilter === 'oportunidades' 
                ? 'bg-slate-800 border border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.1)]' 
                : 'bg-slate-900/80 border border-slate-800 hover:bg-slate-800/80'
            }`}
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
            ...
            <div className="min-w-0">
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Oportunidades</p>
              <p className="text-sm font-extrabold text-white truncate">
                {metrics.totalVolume} <span className="text-[11px] font-normal text-slate-400">negócios</span>
              </p>
            </div>
          </button>

          <div className="bg-slate-900/80 border border-slate-800 rounded-xl px-3.5 py-2.5 flex items-center gap-3">
          ...
          <div className="hidden lg:flex bg-slate-900/80 border border-slate-800 rounded-xl px-3.5 py-2.5 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Copiloto Gemini</p>
              <p className="text-sm font-extrabold text-[#55b0ff] truncate">
                {metrics.iaAssistedCount} <span className="text-[11px] font-normal text-slate-400">qualificados</span>
              </p>
            </div>
          </div>
        </div>
              ...
              {stages.map((stage, stageIdx) => {
                ...
                return (
                  <div key={stage} className="w-80 sm:w-84 flex flex-col h-full max-h-full">
                    ...
                    {/* Stage Header Digify */}
                    <div className="bg-slate-900 border border-slate-800 rounded-t-2xl p-3.5 shrink-0 shadow-sm">
                      <div className="flex justify-between items-center mb-1.5">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          />
                          <h3 className="font-bold text-white text-xs uppercase tracking-wider truncate font-sans">
                            {stage}
                          </h3>
                        </div>
                        <span className="text-[11px] font-extrabold bg-slate-800 text-[#55b0ff] px-2.5 py-0.5 rounded-full border border-slate-700 shrink-0 font-mono">
                          {stageDeals.length}
                        </span>
                      </div>
                      
                      {/* Subtotal da Etapa */}
                      <div className="flex justify-between items-center text-[10px] text-slate-400 pt-1 border-t border-white/[0.04]">
                      ...
                      {(provided, snapshot) => (
                        ...
                        >
                              ...
                              <DraggableItem key={deal.id} draggableId={String(deal.id)} index={index}>
                                {(provided: any, snapshot: any) => (
                                  <div 
                                    ...
                                    className={`bg-slate-900 p-3.5 rounded-xl border transition-all duration-200 cursor-grab select-none group ${
                                      snapshot.isDragging 
                                        ? 'border-[#0a50ff] shadow-[0_16px_36px_rgba(10,80,255,0.3)] rotate-1 scale-[1.02] z-50 ring-2 ring-[#0a50ff]' 
                                        : 'border-slate-800 hover:border-[#0a50ff]/50 hover:shadow-[0_8px_24px_rgba(10,80,255,0.12)] hover:-translate-y-0.5'
                                    }`}
                                  >
                                    ...
                                    <div className="flex items-center gap-2.5 mb-2.5">
                                      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#0a50ff] to-[#55b0ff] flex items-center justify-center text-[10px] font-extrabold text-white shrink-0 shadow-sm">
                                        {getInitials(deal.contato)}
                                      </div>
                                      <div className="min-w-0 flex-1">
                                        <p className="text-xs font-bold text-white truncate group-hover:text-[#55b0ff] transition-colors">
                                          {deal.contato}
                                        </p>
                                    ...
                                    <h4 className="font-semibold text-slate-200 text-xs leading-snug mb-2.5 line-clamp-2">
                                      {deal.titulo}
                                    </h4>
                                    ...
                                    <div className="pt-2.5 border-t border-slate-800 flex justify-between items-center text-[11px]">
                                    ...
                                    <div className="mt-2 pt-2 border-t border-white/[0.04] flex items-center justify-between text-[11px] text-slate-400">
                                      <div className="flex items-center gap-1.5">
                                        <a
                                      ...
                                      {stageIdx < stages.length - 1 && (
                                        <button
                                          ...
                                          className="flex items-center gap-1 text-[10px] font-bold text-slate-400 hover:text-white px-2 py-0.5 rounded-lg hover:bg-[#0a50ff] transition-all"
                          ...
                          {stageDeals.length === 0 && !snapshot.isDraggingOver && (
                            <div className="border border-dashed border-slate-800 rounded-xl h-28 flex flex-col items-center justify-center text-[11px] text-slate-500 bg-slate-900/40 p-3 text-center">
                              <Layers size={18} className="text-slate-600 mb-1" />
      ...
      {selectedDeal && (
        <>
          <div 
            ...
            aria-hidden="true"
          />
          <div className="fixed top-0 right-0 h-full w-full sm:max-w-lg bg-slate-900 border-l border-slate-700 shadow-[0_20px_60px_rgba(0,0,0,0.5)] animate-in slide-in-from-right duration-200 flex flex-col z-50 font-sans">
            
            {/* Header do Drawer */}
            <div className="p-5 border-b border-slate-700 bg-slate-900 flex justify-between items-start shrink-0">
              <div>
            ...
            <div className="px-5 py-3 bg-slate-800 border-b border-slate-700 shrink-0">
              <p className="text-[10px] uppercase font-bold text-slate-400 mb-2">Progresso no Pipeline</p>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 crm-kanban-scroll">
              ...
              {/* Card de Assinante */}
              <div className="bg-slate-900 p-4 rounded-2xl border border-slate-700 space-y-3 text-xs">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#0a50ff] to-[#55b0ff] flex items-center justify-center text-sm font-extrabold text-white">
                    {getInitials(selectedDeal.contato)}
                  </div>
                  <div>
                    <p className="font-bold text-white text-sm">{selectedDeal.contato}</p>
                    <p className="text-slate-400">{selectedDeal.telefone || '(11) 98765-4321'}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <div className="p-2.5 bg-slate-800 rounded-xl border border-slate-700">
                    ...
                    <p className="font-semibold text-white mt-0.5">{selectedDeal.plano || 'Fibra 500MB'}</p>
                  </div>
                  <div className="p-2.5 bg-slate-800 rounded-xl border border-slate-700">
                ...
                <div className="pt-1">
                  ...
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-3 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="bg-slate-900 p-4 rounded-2xl border border-slate-700">
                <h3 className="font-bold text-white mb-3 text-xs flex items-center gap-1.5">
                  <Clock size={14} className="text-[#55b0ff]" /> Histórico de Atividades
                </h3>
                <div className="space-y-3 pl-2 border-l-2 border-slate-700 ml-1.5 text-xs">
                  <div className="relative pl-3">
                    ...
                    <p className="font-semibold text-white">Etapa atual: {selectedDeal.estagio}</p>
                  </div>
                  <div className="relative pl-3">
                    <div className="absolute -left-[17px] top-1 w-2.5 h-2.5 bg-slate-500 rounded-full border-2 border-slate-900" />
                    <p className="text-[10px] text-slate-400">Entrada no Sistema</p>
                    <p className="text-slate-400">Lead sincronizado via API Omnichannel</p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 border-t border-slate-700 bg-slate-900 flex gap-2.5 shrink-0">
              <a
                href={`https://wa.me/55${(selectedDeal.telefone || '').replace(/\D/g, '')}`}
                target="_blank"
                rel="noreferrer"
                className="flex-1 bg-slate-800 hover:bg-slate-800 border border-slate-700 text-white px-3 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2"
              >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-2xl max-w-md w-full p-6 border border-slate-700 shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-in zoom-in-95">
            ...
            <form onSubmit={handleCreateDeal} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-300 mb-1">Título da Oportunidade / Assunto</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-[#0a50ff]"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-300 mb-1">Plano de Internet</label>
                  <select
              ...
              <div>
                <label className="block font-bold text-slate-300 mb-1">Endereço de Instalação</label>
                <input
                  type="text"
              ...
              <div className="pt-3 flex justify-end gap-2 border-t border-slate-700">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2 border border-slate-700 text-slate-400 rounded-xl font-bold hover:bg-slate-900 transition-colors"
                >
                  Cancelar
                </button>
                <button
