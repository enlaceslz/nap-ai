          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-slate-900 p-4 rounded-2xl border border-white/5 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Alarme Óptico (&lt; -26 dBm)</p>
            ...
            <div className="bg-slate-900 rounded-2xl border border-white/5 overflow-hidden">
              {/* FILTROS RÁPIDOS */}
              <div className="flex items-center justify-between flex-wrap gap-2 px-4 py-3 bg-slate-950/40 border-b border-white/5">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-[10px] uppercase font-bold text-slate-500 mr-1">Filtro:</span>
                  <button
                          ...
                          <td className="p-4 text-right">
                            <div className="flex justify-end gap-2 opacity-100 sm:opacity-80 sm:group-hover:opacity-100 transition-opacity">
                              ...
                              <button 
                                ...
                                className="p-2 bg-slate-950 border border-white/5 hover:border-purple-500/30 text-slate-400 hover:text-purple-400 rounded-lg transition-colors disabled:opacity-50" 
          ...
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-xl p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-white/5 space-y-2">
                ...
                {diagTelemetria && (
                  <div className="grid grid-cols-3 gap-2 pt-2 text-[10px] text-slate-400 border-t border-white/5">
                    <div>
                      <span className="text-slate-500 block">Ping Gateway</span>
                      <span className="font-mono text-white font-bold">{diagTelemetria.pingGateway || '1.2 ms'}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block">Ping DNS</span>
                      <span className="font-mono text-white font-bold">{diagTelemetria.pingDnsPrimario || '3.8 ms'}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block">Perda WAN</span>
                  ...
                  <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl space-y-2 animate-in fade-in">
                    ...
                    <div className="grid grid-cols-4 gap-2 pt-1 text-center">
                      <div className="bg-slate-900/80 p-2 rounded-lg border border-white/5">
                        <span className="text-[9px] uppercase font-bold text-slate-500 block">Perda</span>
                        <span className="text-emerald-400 font-mono font-bold text-xs">{pingResult.perda}</span>
                      </div>
                      <div className="bg-slate-900/80 p-2 rounded-lg border border-white/5">
                        <span className="text-[9px] uppercase font-bold text-slate-500 block">Mínimo</span>
                        <span className="text-white font-mono font-bold text-xs">{pingResult.latenciaMinima}</span>
                      </div>
                      <div className="bg-slate-900/80 p-2 rounded-lg border border-white/5">
                        <span className="text-[9px] uppercase font-bold text-slate-500 block">Médio</span>
                        <span className="text-blue-400 font-mono font-bold text-xs">{pingResult.latenciaMedia}</span>
                      </div>
                      <div className="bg-slate-900/80 p-2 rounded-lg border border-white/5">
                        <span className="text-[9px] uppercase font-bold text-slate-500 block">Máximo</span>
                        <span className="text-white font-mono font-bold text-xs">{pingResult.latenciaMaxima}</span>
                      </div>
                    </div>
                  </div>
                ) : (
          ...
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-lg p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleCriarIncidente} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-400 font-bold mb-1">Tipo de Evento NOC</label>
                  <select
                    value={novoTipo}
                    onChange={(e: any) => setNovoTipo(e.target.value)}
                    className="w-full bg-slate-950 border border-white/10 rounded-xl px-3 py-2 text-white outline-none focus:border-red-500 font-medium"
                  >
