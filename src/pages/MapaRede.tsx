...
const createCustomIcon = (status: 'online' | 'offline' | 'alerta') => {
 ...
 <div class="w-1.5 h-1.5 bg-card rounded-full z-10"></div>
 </div>
 `,
 iconSize: [24, 24],
 iconAnchor: [12, 12],
...
const createServiceOrderIcon = (prioridade: string) => {
 ...
 const bgColor = isUrgente ? 'bg-amber-500' : 'bg-accent';
...
export default function MapaRede() {
 ...
 return {
 lat: config.mapa?.centroPadrao?.lat ?? -2.5297,
 lng: config.mapa?.centroPadrao?.lng ?? -44.3028
 };
 ...
 volt: '3.3V',
 wifiSsid: `DJD_${onu.nome.split(' ')[0]}`,
 ...
 return (
 <div className={isFullScreen ? "fixed inset-0 z-[999] flex flex-col bg-background" : "flex-1 flex flex-col h-full bg-background relative overflow-hidden"}>
 ...
 {/* HEADER DE CONTROLE */}
 <div className={`p-6 border-b border-border bg-card/90 backdrop-blur-md z-20 shadow-md flex-shrink-0 transition-all duration-300 ${showControls ? 'max-h-[800px] opacity-100 overflow-visible' : 'max-h-0 opacity-0 overflow-hidden !p-0 !border-0'}`}>
 ...
 <p className="text-sm text-muted-foreground mt-1">Monitoramento geográfico em tempo real das ONTs (TR-069) e Alarmes.</p>
 </div>
 
 <div className="flex flex-wrap gap-3">
 <div 
 className={`bg-background border ${statusFilter === 'todos' ? 'border-white bg-muted ring-2 ring-white/20' : 'border-border'} rounded-xl px-4 py-2 flex flex-col items-center justify-center min-w-[100px] cursor-pointer hover:bg-muted transition-all`} 
 onClick={() => setStatusFilter('todos')}
 >
 <span className="text-xl font-bold text-foreground">{stats.total}</span>
 <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Total Ativos</span>
 </div>
 <div 
 ...
 className="w-full pl-10 pr-4 py-2.5 bg-background border border-border rounded-xl text-sm text-foreground placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
 />
 {searchTerm && (
 <button 
 ...
 <div className="absolute top-full left-0 right-0 mt-1.5 bg-card border border-border rounded-xl shadow-2xl overflow-hidden z-50 divide-y divide-white/5">
 <div className="px-3 py-1.5 bg-background/80 text-[10px] font-bold uppercase tracking-wider text-muted-foreground flex items-center justify-between">
 ...
 className="w-full text-left px-3 py-2 hover:bg-accent transition-colors flex items-center justify-between gap-2 group cursor-pointer"
 >
 ...
 MAC: <span className="font-mono text-muted-foreground">{match.mac}</span> • {match.olt}
 </div>
 </div>
 ...
 <Crosshair size={14} className="text-muted-foreground group-hover:text-blue-400 transition-colors" />
 </div>
 </button>
 ))}
 </div>
 ...
 className="w-full pl-10 pr-10 py-2.5 bg-background border border-border rounded-xl text-sm text-foreground placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
 />
 <button 
 type="submit"
 ...
 className="bg-background hover:bg-card text-blue-500 border border-border p-2.5 rounded-xl transition-colors cursor-pointer flex items-center justify-center shadow-lg"
 >
 <Navigation size={20} />
 </button>
 </div>
 ...
 <div className="flex items-center bg-background border border-border rounded-xl p-0.5 flex-shrink-0">
 <button
 type="button"
 ...
 >
 <Wrench size={14} className={showTechniciansLayer ? 'text-indigo-400' : 'text-muted-foreground'} />
 <span>Técnicos & OSs</span>
 {tecnicos.length > 0 && (
 <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-mono font-bold ${
 showTechniciansLayer ? 'bg-indigo-500 text-white' : 'bg-muted text-muted-foreground'
 }`}>
 {tecnicos.length}
 </span>
 )}
 ...
 className="bg-background border border-border rounded-xl px-4 py-2.5 text-sm text-foreground focus:outline-none focus:border-blue-500 flex-1 xl:w-48 appearance-none cursor-pointer"
 >
 ...
 className="bg-muted hover:bg-accent text-foreground p-2.5 rounded-xl border border-border transition-colors flex-shrink-0 cursor-pointer"
 ...
 {/* ÁREA DO MAPA */}
 <div className="flex-1 relative z-10 bg-card min-h-0">
 ...
 <p className="text-muted-foreground font-medium">Carregando dados geolocalizados do GenieACS...</p>
 </div>
 ) : null}

 {/* Legenda Interativa Flutuante */}
 <div className="absolute bottom-6 right-6 z-[400] bg-card/90 backdrop-blur-md border border-border p-4 rounded-xl shadow-2xl flex flex-col gap-3 min-w-[200px]">
 <div className="flex justify-between items-center mb-1">
 <h3 className="text-sm font-bold text-foreground">Legenda (Status TR-069)</h3>
 <button 
 ...
 <>
 <div 
 className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${statusFilter === 'online' ? 'bg-white/10' : 'hover:bg-accent'}`}
 ...
 <div className="w-1.5 h-1.5 bg-card rounded-full z-10"></div>
 </div>
 <div className="flex-1">
 <p className="text-xs font-bold text-foreground leading-none">Online</p>
 <p className="text-[10px] text-emerald-400">Sinal e conexão OK</p>
 </div>
 <span className="text-xs font-mono font-bold text-muted-foreground">{stats.online}</span>
 </div>

 <div 
 className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${statusFilter === 'alerta' ? 'bg-white/10' : 'hover:bg-accent'}`}
 ...
 <div className="flex-1">
 <p className="text-xs font-bold text-foreground leading-none">Atenção</p>
 <p className="text-[10px] text-amber-400">Sinal Crítico (Atenuado)</p>
 </div>
 <span className="text-xs font-mono font-bold text-muted-foreground">{stats.alertas}</span>
 </div>

 <div 
 className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${statusFilter === 'offline' ? 'bg-white/10' : 'hover:bg-accent'}`}
 ...
 <div className="flex-1">
 <p className="text-xs font-bold text-foreground leading-none">Offline (LOS)</p>
 <p className="text-[10px] text-red-400">Rompimento / Sem Energia</p>
 </div>
 <span className="text-xs font-mono font-bold text-muted-foreground">{stats.offline}</span>
 </div>

 {/* Seção Campo (quando ativada) */}
 {showTechniciansLayer && (
 <div className="pt-2 border-t border-border flex flex-col gap-2">
 <span className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Equipes de Rua</span>
 ...
 <span className="ml-auto font-mono text-muted-foreground font-bold">{tecnicos.length}</span>
 </div>
 <div className="flex items-center gap-2.5 text-xs text-muted-foreground">
 ...
 <span className="ml-auto font-mono text-muted-foreground font-bold">{ordensServico.length}</span>
 </div>
 </div>
 )}
 </>
 ...
 <span className="font-bold text-foreground text-sm">{ont.id}</span>
 ...
 <p><strong className="text-foreground">Cliente:</strong> {ont.cliente}</p>
 <p><strong className="text-foreground">MAC:</strong> <span className="font-mono">{ont.mac}</span></p>
 <p><strong className="text-foreground">Plano:</strong> {ont.plano}</p>

 <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-border">
 <div>
 <strong className="text-foreground block mb-0.5">OLT / PON:</strong>
 <span className="font-mono text-[10px] bg-card border border-border px-1.5 py-0.5 rounded text-muted-foreground">{ont.olt} - PON {ont.pon}</span>
 </div>
 <div>
 <strong className="text-foreground block mb-0.5">Sinal Óptico:</strong>
 ...
 className="w-full bg-muted hover:bg-accent text-card-foreground py-1.5 rounded-lg text-xs font-bold transition-colors border border-border flex items-center justify-center gap-1 cursor-pointer"
 >
 ...
 <div>
 <h4 className="font-bold text-foreground text-xs leading-none">{tec.nome}</h4>
 <span className="text-[10px] text-muted-foreground">{tec.veiculo}</span>
 </div>
 ...
 <span className="font-mono font-bold text-foreground">{tec.velocidade_kmh} km/h</span>
 </div>
 <div className="flex items-center justify-between text-[11px]">
 <span className="text-muted-foreground">Bateria do PWA:</span>
 ...
 <span className="font-mono text-muted-foreground">±{tec.precisao_metros}m</span>
 </div>
 ...
 <p><strong className="text-foreground">Tipo:</strong> {os.tipo}</p>
 <p><strong className="text-foreground">Cliente:</strong> {os.cliente_nome}</p>
 ...
 type="button"
 onClick={() => navigate('/admin/campo')}
 className="mt-3 w-full bg-muted hover:bg-accent text-foreground py-1.5 rounded-lg text-xs font-bold transition-colors border border-border flex items-center justify-center gap-1 cursor-pointer"
 >
 <ExternalLink size={12} />
 ...
 {selectedOnt && (
 <div className="absolute top-4 right-4 bottom-4 w-96 max-w-[calc(100vw-32px)] z-[500] bg-card/95 backdrop-blur-xl border border-border rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-right-8 duration-200">
 {/* Header do Painel */}
 <div className="p-4 border-b border-border bg-background/60 flex items-center justify-between flex-shrink-0">
 ...
 <div>
 <div className="flex items-center gap-2">
 <span className="font-bold text-foreground text-sm">{selectedOnt.id}</span>
 ...
 <p className="text-[11px] text-muted-foreground">{selectedOnt.modelo}</p>
 </div>
 </div>
 ...
 className="w-7 h-7 rounded-lg bg-white/5 hover:bg-accent text-muted-foreground hover:text-foreground flex items-center justify-center transition-colors cursor-pointer"
 title="Fechar Painel"
 >
 <X size={16} />
 </button>
 ...
 <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Assinante & Conexão</div>
 <div className="flex justify-between items-start">
 <span className="text-foreground font-semibold text-sm">{selectedOnt.cliente}</span>
 ...
 <span className="font-mono text-card-foreground font-medium">{selectedOnt.ip}</span>
 </div>
 <div>
 <span className="text-muted-foreground block">Uptime:</span>
 <span className="text-card-foreground font-medium">{selectedOnt.uptime}</span>
 </div>
 </div>
 <div className="pt-1 border-t border-border flex items-center justify-between">
 <span className="text-muted-foreground text-[11px]">MAC Address:</span>
 <div className="flex items-center gap-1.5">
 <span className="font-mono text-card-foreground text-[11px] bg-card px-1.5 py-0.5 rounded border border-border">
 {selectedOnt.mac}
 </span>
 <button
 type="button"
 onClick={() => handleCopyMac(selectedOnt.mac)}
 className="text-muted-foreground hover:text-foreground p-1 rounded hover:bg-accent transition-colors cursor-pointer"
 title="Copiar MAC"
 >
 ...
 <div className="bg-background/60 border border-border rounded-xl p-3 space-y-2">
 <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Topologia de Fibra</div>
 <div className="flex items-center justify-between text-[11px]">
 <span className="text-muted-foreground">OLT Concentradora:</span>
 <span className="font-medium text-card-foreground">{selectedOnt.olt}</span>
 </div>
 <div className="flex items-center justify-between text-[11px]">
 <span className="text-muted-foreground">Porta PON / Slot:</span>
 ...
 <div className="bg-background/60 border border-border rounded-xl p-3 space-y-3">
 <div className="flex items-center justify-between">
 <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1">
 <Signal size={12} className="text-blue-400" /> Níveis Ópticos (TR-069)
 </span>
 <span className="text-[10px] text-muted-foreground font-mono">Calibrado</span>
 </div>

 <div className="grid grid-cols-2 gap-2">
 {/* RX Power */}
 <div className="bg-card/80 border border-border p-2.5 rounded-xl">
 <span className="text-[10px] text-muted-foreground block mb-1">Potência RX (ONU)</span>
 ...
 <span className="text-[9px] text-muted-foreground block mt-0.5">Ref: -14 a -25 dBm</span>
 </div>

 {/* TX Power */}
 <div className="bg-card/80 border border-border p-2.5 rounded-xl">
 <span className="text-[10px] text-muted-foreground block mb-1">Potência TX (Laser)</span>
 <span className="text-base font-bold font-mono text-card-foreground">
 {selectedOnt.txPower}
 </span>
 <span className="text-[9px] text-muted-foreground block mt-0.5">Ref: +1.5 a +5 dBm</span>
 </div>

 {/* Temperatura */}
 <div className="bg-card/80 border border-border p-2.5 rounded-xl">
 <div className="flex items-center justify-between mb-1">
 <span className="text-[10px] text-muted-foreground">Temp. Interna</span>
 <Thermometer size={12} className="text-amber-400" />
 </div>
 <span className="text-sm font-bold font-mono text-card-foreground">{selectedOnt.temp}</span>
 </div>

 {/* Tensão */}
 <div className="bg-card/80 border border-border p-2.5 rounded-xl">
 <div className="flex items-center justify-between mb-1">
 <span className="text-[10px] text-muted-foreground">Tensão VCC</span>
 <Zap size={12} className="text-blue-400" />
 </div>
 <span className="text-sm font-bold font-mono text-card-foreground">{selectedOnt.volt}</span>
 </div>
 </div>
 ...
 <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Comandos & Diagnóstico</div>
 ...
 className="w-full bg-muted hover:bg-accent text-foreground py-2 px-3 rounded-xl font-semibold text-xs border border-border flex items-center justify-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
 >
 ...
 className="w-full bg-muted hover:bg-red-500/20 text-card-foreground hover:text-red-300 py-2 px-3 rounded-xl font-semibold text-xs border border-border hover:border-red-500/30 flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
 >
 ...
 className="w-full bg-background hover:bg-accent text-muted-foreground hover:text-foreground py-2 px-3 rounded-xl font-medium text-xs border border-border flex items-center justify-center gap-2 transition-colors cursor-pointer"
 >
 <Activity size={13} />
 ...
 <div>
 <h3 className="font-bold text-foreground text-sm leading-tight">{selectedTecnico.nome}</h3>
 <span className="text-[11px] text-muted-foreground">Equipe de Campo • {selectedTecnico.veiculo}</span>
 </div>
 </div>
 <button 
 type="button"
 onClick={() => setSelectedTecnico(null)}
 className="text-muted-foreground hover:text-foreground p-1.5 rounded-lg hover:bg-accent transition-colors cursor-pointer"
 >
 <X size={18} />
 </button>
 </div>
 ...
 <div className="flex items-center justify-between">
 <span className="text-muted-foreground">Telemetria GPS:</span>
 <span className="text-card-foreground font-mono">±{selectedTecnico.precisao_metros} metros</span>
 </div>
 <div className="flex items-center justify-between">
 <span className="text-muted-foreground">Velocidade em Trânsito:</span>
 <span className="font-bold text-foreground font-mono">{selectedTecnico.velocidade_kmh} km/h</span>
 </div>
 <div className="flex items-center justify-between">
 <span className="text-muted-foreground">Bateria do Smartphone:</span>
 <span className="text-emerald-400 font-bold font-mono">{selectedTecnico.bateria}%</span>
 </div>
 <div className="pt-2 border-t border-border">
 <span className="text-muted-foreground block mb-1">Última Localização Registrada:</span>
 <p className="text-card-foreground font-medium">{selectedTecnico.endereco}</p>
 </div>
 </div>
 ...
 <div>
 <h3 className="font-bold text-foreground text-sm leading-tight">OS #{selectedOS.numero}</h3>
 <span className="text-[11px] text-muted-foreground">{selectedOS.tipo}</span>
 </div>
 </div>
 <button 
 type="button"
 ...
 <span className="font-bold text-foreground">{selectedOS.cliente_nome}</span>
 </div>
 <div className="flex items-center justify-between">
 <span className="text-muted-foreground">Técnico Encarregado:</span>
 <span className="font-medium text-indigo-300">{selectedOS.tecnico_nome || 'Aguardando Despacho'}</span>
 </div>
 <div className="pt-2 border-t border-border">
 <span className="text-muted-foreground block mb-1">Endereço do Atendimento:</span>
 <p className="text-card-foreground font-medium">{selectedOS.endereco} - {selectedOS.bairro}</p>
 </div>
 </div>
 ...
 className="w-full bg-muted hover:bg-accent text-foreground py-2.5 px-3 rounded-xl font-semibold text-xs border border-border flex items-center justify-center gap-2 transition-all cursor-pointer"
 >
 <Activity size={14} />
 ...
 {toastMessage && (
 <div className="absolute top-6 left-1/2 -translate-x-1/2 z-[600] bg-card border border-blue-500/40 text-foreground px-4 py-2.5 rounded-xl shadow-2xl flex items-center gap-2.5 text-xs font-medium animate-in fade-in-50 zoom-in-95">
