...
const carregarOrdens = async () => {
 ...
 bairro: 'Desconhecido',
 cidade: 'Sede',
 lat: (o.location && typeof o.location === 'object' ? o.location.lat : o.lat) || -2.5297,
 lng: (o.location && typeof o.location === 'object' ? o.location.lng : o.lng) || -44.3028,
 status: o.status,
 ...
 <div className="bg-card border border-border rounded-2xl p-4 md:p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
 ...
 <h1 className="text-xl font-bold text-foreground font-outfit">Modo Técnico de Campo</h1>
 ...
 Operador logado: <strong className="text-card-foreground">{user?.name}</strong> • Veículo: <span className="text-emerald-300">{user?.veiculo || 'Fiorino 01'}</span>
 </p>
 </div>
 </div>

 {/* Telemetria GPS em Tempo Real */}
 <div className="flex items-center gap-3 bg-background border border-border px-4 py-2.5 rounded-xl w-full md:w-auto justify-between">
 ...
 <div>
 <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">GPS Ativo (Backgr.)</div>
...
{/* Coluna Esquerda: Lista de OSs */}
 ...
 selectedOS?.id === os.id 
 ? 'bg-card border-blue-500/50 shadow-lg' 
 ...
 <h3 className="text-sm font-bold text-foreground leading-tight">{os.cliente_nome}</h3>
 </div>
 ...
 <span className={`font-bold ${os.prioridade === 'urgente' ? 'text-red-400' : 'text-muted-foreground'}`}>
 Tipo: {os.tipo}
 </span>
 </div>
 </div>
 ...
 className="lg:hidden flex items-center gap-1.5 text-muted-foreground font-bold bg-white/5 hover:bg-accent border border-border py-2 px-3 rounded-xl transition-colors mb-2 w-fit"
 >
 ...
 <span className="text-xs bg-white/5 border border-border text-muted-foreground px-2 py-0.5 rounded font-bold">
 {selectedOS.tipo}
 </span>
 </div>
 <h2 className="text-xl font-bold text-foreground mt-1">{selectedOS.cliente_nome}</h2>
 <p className="text-xs text-muted-foreground">CPF: {selectedOS.cliente_cpf} • Agendamento: {selectedOS.horario_agendado}</p>
 </div>
 </div>
 ...
 <div>
 <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Endereço de Atendimento</h4>
 <p className="text-sm font-semibold text-foreground">{selectedOS.endereco}</p>
 <p className="text-xs text-muted-foreground">{selectedOS.bairro} - {selectedOS.cidade}</p>
 </div>
 </div>
 </div>
 
 {/* Visualização de Rota Integrada */}
 <div className="flex-1 h-32 rounded-xl overflow-hidden border border-border relative z-0">
 <MapContainer 
 ...
 <div className="absolute top-2 right-2 bg-card/90 backdrop-blur border border-border text-[10px] px-2 py-1 rounded-md z-[400] text-blue-400 font-bold shadow-md">
 GPS Integrado
 </div>
 </div>
 </div>
 ...
 : 'bg-white/5 text-muted-foreground hover:bg-accent hover:text-foreground border border-border hover:border-border'
 }`}
 >
 ...
 <span className="text-[10px] text-muted-foreground uppercase font-bold block">Potência RX Óptica</span>
 ...
 <span className="text-[10px] text-muted-foreground block">Ideal: -18 a -24 dBm</span>
 </div>

 <div className="p-3 bg-card border border-border rounded-xl">
 <span className="text-[10px] text-muted-foreground uppercase font-bold block">Serial da ONU</span>
 <span className="text-xs font-mono font-bold text-card-foreground block truncate">
 {selectedOS.onu_serial || 'ZTEG12345678'}
 </span>
 <span className="text-[10px] text-emerald-400">Provisionada TR-069</span>
 </div>

 <div className="p-3 bg-card border border-border rounded-xl col-span-2 sm:col-span-1">
 <span className="text-[10px] text-muted-foreground uppercase font-bold block">MAC da CPE</span>
 <span className="text-xs font-mono font-bold text-card-foreground block truncate">
 {selectedOS.onu_mac || 'E0:67:B3:91:AA:12'}
 </span>
 <span className="text-[10px] text-muted-foreground">Wi-Fi 6 Ativo</span>
 </div>
 </div>
 </div>
 ...
 <label 
 key={item.key}
 className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-accent cursor-pointer transition-colors"
 >
 <input 
 type="checkbox"
 ...
 <button onClick={() => setShowAssinaturaModal(false)} className="text-muted-foreground hover:text-foreground p-1">
 <X size={20} />
 </button>
 </div>
 ...
 <div className="space-y-2">
 <label className="text-xs font-bold text-muted-foreground uppercase">1. Foto da Instalação (CTO/ONU)</label>
 {!fotoInstalacao ? (
 <label className="flex flex-col items-center justify-center h-32 border-2 border-dashed border-border rounded-xl bg-white/5 cursor-pointer hover:bg-accent transition-colors">
 <Camera size={32} className="text-muted-foreground mb-2" />
 <span className="text-sm font-bold text-muted-foreground">Tirar Foto</span>
 ...
 <div className="space-y-2">
 <div className="flex items-center justify-between">
 <label className="text-xs font-bold text-muted-foreground uppercase">2. Assinatura do Cliente</label>
 <button onClick={clearSignature} className="text-xs text-muted-foreground hover:text-foreground">Limpar</button>
 </div>
 <div className="bg-background border border-border rounded-xl overflow-hidden relative touch-none">
 <canvas 
 ref={canvasRef}
 width={400}
 height={150}
 ...
 <span className="text-[10px] text-muted-foreground font-mono">Assine acima</span>
 </div>
 </div>
 </div>
 </div>
