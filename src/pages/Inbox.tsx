          ...
          if (data && data.length > 0) {
            const merged = data.map((bChat: any) => {
               ...
               return {
                 ...
                 protocolo: `NAP-${new Date().getFullYear()}-${bChat.id}`,
    ...
    <div className="flex-1 flex h-full bg-slate-950 overflow-hidden relative font-sans">
              ...
              return (
                <div 
                  ...
                  className={`p-4 cursor-pointer transition-colors border-l-[3px] ${
                    isSelected 
                      ? 'bg-slate-800 border-l-blue-500 shadow-sm' 
      ...
      {activeChat ? (
        <div className={`flex-1 flex-col bg-slate-950 relative ${activeChatId ? 'flex' : 'hidden md:flex'} overflow-hidden`}>
          ...
          <header className="h-16 border-b border-white/5 bg-slate-900 px-4 sm:px-6 flex items-center justify-between shrink-0 z-20">
              ...
              {activeChat.status === 'triagem_ia' ? (
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 border border-white/10 rounded-xl transition-all"
          ...
          <div className="flex-1 flex overflow-hidden relative">
            ...
            {/* Thread de Mensagens */}
            <div 
              className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-slate-950" 
              style={{ 
                scrollbarWidth: 'thin'
              }}
            >
                  ...
                  >
                    <div 
                      className={`max-w-[85%] sm:max-w-[70%] rounded-2xl p-4 text-xs sm:text-sm leading-relaxed border ${
                        isNote 
                          ...
                          : isClient
                            ? 'bg-slate-900 border-white/5 text-slate-200 rounded-tl-sm shadow-sm'
                            : isAi
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between gap-3 mb-1.5 pb-1.5 border-b border-white/10">
                        ...
                        <div className={`flex items-center gap-1 text-[10px] font-mono ${isClient || isNote || isAi ? 'text-slate-400' : 'text-blue-200'}`}>
            ...
            {isSgpDrawerOpen && (
              <aside className="absolute right-0 top-0 bottom-0 z-40 lg:relative lg:top-0 lg:z-auto w-80 lg:w-96 border-l border-white/10 bg-slate-900 overflow-y-auto p-4 space-y-4 shrink-0 animate-in slide-in-from-right-3 duration-200 shadow-2xl lg:shadow-none">
          ...
          <footer className="border-t border-white/5 bg-slate-900 p-3 sm:p-4 z-20 space-y-2 shrink-0">
            ...
            {activeChat.status === 'triagem_ia' && (
              <div className="flex flex-wrap items-center justify-between gap-2 p-2 px-3 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-slate-400 hidden sm:inline">• Você pode intervir a qualquer instante</span>
                </div>
                <button
                  type="button"
                  onClick={() => handleAssumeChat(activeChat.id)}
              ...
              <div className="flex rounded-xl bg-white/[0.02] p-0.5 text-xs font-semibold">
                <button
                  ...
                  className={`px-3 py-1.5 text-[11px] rounded-lg transition-all flex items-center gap-1.5 ${
                    !isInternalNote ? 'bg-slate-800 text-blue-400 font-bold border border-blue-500/30 shadow-sm' : 'text-slate-400 hover:text-white hover:bg-slate-800 border border-transparent'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-[11px] rounded-lg transition-all flex items-center gap-1.5 ${
                    isInternalNote ? 'bg-amber-500/10 text-amber-400 font-bold border border-amber-500/30 shadow-sm' : 'text-slate-400 hover:text-white hover:bg-slate-800 border border-transparent'
                  }`}
                >
            ...
            <div className={`flex items-end gap-2 p-1.5 rounded-2xl border transition-all ${
              activeChat.status === 'triagem_ia'
                ...
                : isInternalNote 
                  ...
                  : 'bg-slate-950 border-white/10 focus-within:border-blue-500/50'
            }`}>
              ...
              <button
                ...
                className={`w-10 h-10 flex items-center justify-center rounded-full text-white font-bold transition-all active:scale-95 disabled:opacity-40 shrink-0 shadow-sm ${
                  activeChat.status === 'triagem_ia'
