    ...
    <div className="flex flex-col h-full bg-slate-950 relative">
      <div className="flex items-center justify-between px-6 py-4 bg-slate-900 border-b border-slate-800 z-10">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Georreferenciamento e Engenharia Física Nativa</p>
        </div>
        <div className="flex gap-2 bg-slate-950 p-1 rounded-lg border border-slate-800">
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded text-xs font-semibold ${activeLayer === 'all' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'}`}
          >
            Todas as Camadas
          </button>
          <button 
            onClick={() => setActiveLayer('layer-olt')}
            className={`px-3 py-1.5 rounded text-xs font-semibold flex items-center gap-1 ${activeLayer === 'layer-olt' ? 'bg-fuchsia-600 text-white' : 'text-slate-400 hover:text-white'}`}
          >
            <Server size={14} /> OLTs
          </button>
          <button 
            onClick={() => setActiveLayer('layer-ont')}
            className={`px-3 py-1.5 rounded text-xs font-semibold flex items-center gap-1 ${activeLayer === 'layer-ont' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'}`}
          >
                ...
                <Popup className="gis-popup text-slate-900">
                  <div className="p-1 min-w-[200px]">
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      <div className="flex justify-between"><span className="text-slate-500">Status:</span><span className={`font-bold ${feature.properties.status === 'online' ? 'text-emerald-500' : feature.properties.status === 'blocked_sgp' ? 'text-purple-500' : 'text-red-500'}`}>{typeof feature.properties.status === "string" ? feature.properties.status.toUpperCase() : "DESCONHECIDO"}</span></div>
                      ...
                      {feature.properties.rx_power !== undefined && (
                        <div className="flex justify-between">
                          <span className="text-slate-500">Sinal Óptico:</span>
