...
export default function CRM() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-950 overflow-hidden text-slate-100 font-sans relative">
      
      {/* Top Header estilo Digify */}
      <div className="p-5 border-b border-slate-800 bg-slate-900/95 backdrop-blur-md shrink-0 space-y-3.5 z-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-extrabold text-white">Base de Clientes (CRM 360)</h1>
        ...
        <div className="fixed bottom-6 right-6 z-50 bg-emerald-500 text-white px-4 py-3 rounded-2xl shadow-xl font-bold animate-in slide-in-from-bottom-5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-300 px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-colors shadow-xs"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-1">
          ...
          >
            ...
            <div>
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Total de Assinantes</p>
              <p className="text-sm font-extrabold text-white">{metrics.total} cadastrados</p>
            </div>
          </button>
          ...
          <button 
            ...
            className={`text-left rounded-xl px-3.5 py-2 flex items-center gap-3 transition-all ${
              statusFilter === 'ativo' 
                ? 'bg-slate-800 border border-[#22c55e]/50 shadow-[0_0_15px_rgba(34,197,94,0.1)]' 
                : 'bg-slate-900/80 border border-slate-800 hover:bg-slate-800/80'
            }`}
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
            ...
            <div>
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Clientes Ativos</p>
          ...
          <button 
            ...
            className={`text-left rounded-xl px-3.5 py-2 flex items-center gap-3 transition-all ${
              statusFilter === 'bloqueado' 
                ? 'bg-slate-800 border border-[#ff5c7a]/50 shadow-[0_0_15px_rgba(255,92,122,0.1)]' 
                : 'bg-slate-900/80 border border-slate-800 hover:bg-slate-800/80'
            }`}
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
            ...
            <div>
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Bloqueados / Débito</p>
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-xl px-3.5 py-2 flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Desbloqueio Ativo</p>
      ...
      <div className="p-4 sm:p-5 flex-1 overflow-y-auto crm-kanban-scroll">
        {crmView === 'consulta_avancada_sgp' ? (
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden flex flex-col shadow-sm">
            
            {/* Toolbar com Busca e Filtro de Status */}
            <div className="p-4 border-b border-slate-800 bg-slate-900 flex flex-wrap gap-3 items-center justify-between">
              <div className="relative flex-1 min-w-[260px] max-w-md">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-900 hover:bg-slate-800 border border-slate-700 rounded-xl text-xs text-white outline-none focus:border-[#0a50ff] focus:ring-1 focus:ring-[#0a50ff] placeholder:text-slate-500 transition-all"
                />
              </div>

              {/* Status Filter Tabs */}
              <div className="flex items-center gap-1 bg-slate-900 p-0.5 rounded-xl border border-slate-700 text-xs">
                <button
                  onClick={() => setStatusFilter('all')}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-300">
                ...
                <tbody className="divide-y divide-slate-800/60 bg-slate-900">
                  {loading ? (
                    <tr><td colSpan={8} className="text-center py-10 text-slate-500">Sincronizando clientes com o SGP ERP...</td></tr>
                  ) : filteredContatos.length === 0 ? (
                    <tr><td colSpan={8} className="text-center py-10 text-slate-500">Nenhum assinante encontrado para o filtro atual.</td></tr>
                  ) : (
                      ...
                      >
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#0a50ff] to-[#55b0ff] flex items-center justify-center text-[10px] font-extrabold text-white shrink-0">
                              {getInitials(contato.nome)}
                            </div>
                            <span className="font-bold text-white group-hover:text-[#55b0ff] transition-colors">
                              {contato.nome}
                            </span>
                          </div>
                        </td>
                        <td 
                          onClick={() => setSelectedContato(contato)}
                          className="px-5 py-3.5 font-mono text-slate-400 text-[11px]"
                        >
                          {contato.cpf_cnpj}
                        </td>
                        <td 
                          onClick={() => setSelectedContato(contato)}
                          className="px-5 py-3.5 font-mono text-slate-300 text-[11px]"
                        >
                          {contato.telefone}
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            <div className="max-w-[180px] truncate">
                              <p className="text-slate-200 truncate" title={contato.endereco || 'Endereço não informado'}>
                        ...
                        <td 
                          ...
                          className="px-5 py-3.5"
                        >
                          <span className="bg-slate-800 border border-slate-700 text-slate-200 px-2 py-0.5 rounded-md text-[10px] font-bold">
                        ...
                        <td className="px-5 py-3.5 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            <button 
                              onClick={() => setSelectedContato(contato)}
                              className="px-2 py-1 bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-700 rounded-lg text-[11px] font-semibold transition-colors"
                              title="Ver ficha 360"
                            >
                              Ficha
                            </button>
            ...
            <div className="p-3.5 border-t border-slate-800 bg-slate-900 flex items-center justify-between text-xs text-slate-400 font-medium">
              <span>Exibindo {filteredContatos.length} de {contatos.length} assinantes cadastrados</span>
              <div className="flex gap-2">
                <button className="px-3 py-1 bg-slate-900 border border-slate-700 rounded-lg hover:bg-slate-800 transition-colors text-slate-300 text-xs">Anterior</button>
                <button className="px-3 py-1 bg-slate-900 border border-slate-700 rounded-lg hover:bg-slate-800 transition-colors text-slate-300 text-xs">Próxima</button>
              </div>
            </div>
          </div>
        )}
      ...
      {selectedContato && (
        <>
          <div 
            ...
            aria-hidden="true"
          />
          <div className="fixed top-0 right-0 h-full w-full sm:max-w-xl bg-slate-900 border-l border-slate-700 shadow-[0_20px_60px_rgba(0,0,0,0.5)] animate-in slide-in-from-right duration-200 flex flex-col z-50 font-sans">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    <h2 className="text-base font-extrabold text-white">{selectedContato.nome}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 crm-kanban-scroll">
              ...
              <div className="border border-slate-700 rounded-2xl overflow-hidden shadow-sm bg-slate-900">
                <SgpAdvancedSearch 
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl divide-y divide-slate-700">
                  <div className="p-3 flex justify-between items-center text-xs">
                    <div className="flex gap-2.5 items-center">
                      ...
                      <div>
                        <p className="font-bold text-white">Suporte Técnico N1</p>
                        <p className="text-[10px] text-slate-400">Atendido por: Lucas Almeida (Ramal 2004)</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] font-bold text-slate-400">Hoje, 09:30</p>
                      <p className="text-[10px] text-slate-500 font-mono">03m 45s</p>
                    </div>
                  </div>
                </div>
              </div>
