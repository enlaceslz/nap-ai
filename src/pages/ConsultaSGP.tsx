...
export default function ConsultaSGP() {
 ...
 {/* Header */}
 <div className="p-6 border-b border-border bg-card z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
 <div>
 ...
 className="px-4 py-2 bg-background hover:bg-accent text-muted-foreground border border-border rounded-xl text-xs font-bold transition-all flex items-center gap-2 shrink-0"
 >
 ...
 {/* Search Bar */}
 <div className="bg-card rounded-2xl border border-border p-6 mb-8 max-w-5xl mx-auto">
 ...
 className="w-full pl-12 pr-4 py-4 bg-background border border-border rounded-xl text-foreground outline-none focus:ring-2 focus:ring-blue-600/50 focus:border-blue-600 transition-all font-medium placeholder:text-muted-foreground "
 />
 </div>
 <button 
 type="submit"
 ...
 <p className="font-bold text-muted-foreground">Sincronizando com {erpNome}...</p>
 </div>
 )}
 ...
 <div key={res.id} className="bg-card rounded-3xl border border-border -200/40 overflow-hidden">
 
 {/* Profile Header */}
 <div className="p-8 border-b border-border bg-gradient-to-br from-slate-900 to-slate-800 text-foreground relative overflow-hidden">
 <div className="absolute inset-0 opacity-10 bg-[radial-gradient(#ffffff_1px,transparent_1px)] [background-size:20px_20px]"></div>
 <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
 <div className="flex items-center gap-5">
 <div className="w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center text-2xl font-bold font-outfit border-2 border-border">
 {res.nome.charAt(0)}
 </div>
 <div>
 ...
 <span className="flex items-center gap-1.5 bg-card/10 px-3 py-1.5 rounded-lg backdrop-blur-md">
 ...
 <div className="bg-card/10 backdrop-blur-md border border-border rounded-xl p-4 min-w-[120px]">
 ...
 {/* Tabs Navigation */}
 <div className="flex overflow-x-auto border-b border-border bg-background/80 px-4">
 {[
 ...
 activeTab === tab.id 
 ? 'border-blue-600 text-blue-400 bg-card' 
 : 'border-transparent text-muted-foreground hover:text-card-foreground hover:bg-accent/50'
 }`}
 >
 {tab.icon} {tab.label}
 </button>
 ...
 <h3 className="font-bold text-foreground font-outfit mb-4 text-sm flex items-center gap-2 border-b border-border pb-2">
 <User size={16} className="text-blue-400" /> Dados de Contato
 </h3>
 <div className="space-y-4">
 <div>
 <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">Telefone Principal (WhatsApp)</p>
 <div className="flex items-center justify-between bg-card border border-border px-3 py-2 rounded-lg">
 <span className="font-bold text-foreground">{res.contato.telefone}</span>
 ...
 <div>
 <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">E-mail Cadastrado</p>
 <p className="font-medium text-foreground text-sm bg-card border border-border px-3 py-2 rounded-lg truncate">{res.contato.email}</p>
 </div>
 <div>
 <div className="flex items-center justify-between mb-1">
 <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Endereço de Instalação</p>
 <button
 type="button"
 ...
 <div className="bg-card border border-border px-3 py-2.5 rounded-lg space-y-2">
 <p className="font-medium text-foreground text-sm leading-relaxed">{res.endereco}</p>
 ...
 <h3 className="font-bold text-foreground font-outfit mb-6 text-sm flex items-center gap-2 border-b border-border pb-3">
 <Activity size={16} className="text-blue-400" /> Histórico de Atendimentos (Últimos 30 dias)
 </h3>
 
 {res.chamados_recentes.map((chamado: any) => (
 <div key={chamado.id} className="relative pl-6 pb-6 border-l-2 border-border last:border-0 last:pb-0">
 ...
 <span className="bg-white/10 text-muted-foreground text-[10px] px-2 py-0.5 rounded font-bold">Ticket #{chamado.id}</span>
 <span className="text-xs text-muted-foreground font-bold">{new Date(chamado.data).toLocaleDateString('pt-BR')}</span>
 </div>
 <span className="text-[10px] uppercase font-bold text-emerald-600 tracking-wider">Resolvido</span>
 </div>
 <p className="text-sm font-medium text-foreground">{chamado.assunto}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 <h3 className="font-bold text-foreground font-outfit text-sm flex items-center gap-2 border-b border-border pb-2">
 ...
 className="w-full bg-card hover:bg-muted border border-border text-muted-foreground py-2.5 px-3 rounded-xl font-bold transition-all text-xs flex items-center justify-center gap-2 active:scale-95"
 >
 <Copy size={15} /> 
 ...
 className="w-full bg-card hover:bg-accent border border-border text-muted-foreground py-2.5 px-3 rounded-xl font-bold transition-all text-xs flex items-center justify-center gap-2"
 >
 ...
 <div className="p-3 bg-card/60 rounded-xl border border-border space-y-1 text-[11px] text-muted-foreground">
 ...
 fatura.status === 'atrasado' ? 'bg-red-50/30 border-red-200' : 'bg-card border-border'
 ...
 <h4 className="font-bold text-foreground text-lg">Fatura #{fatura.id}</h4>
 ...
 <p className="text-sm font-medium text-muted-foreground">Vencimento: {new Date(fatura.vencimento).toLocaleDateString('pt-BR')}</p>
 </div>
 </div>
 <div className="text-right">
 <p className="text-2xl font-bold font-outfit text-foreground">R$ {fatura.valor.toFixed(2).replace('.', ',')}</p>
 ...
 <p className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider mb-2">Linha Digitável / PIX Copia e Cola</p>
 <div className="flex gap-2">
 <input 
 readOnly 
 value={fatura.linha_digitavel}
 className="flex-1 bg-card border border-border rounded-lg px-3 py-2 text-xs font-mono text-muted-foreground outline-none"
 />
 <button 
 onClick={() => copyToClipboard(fatura.linha_digitavel)}
 className="px-4 py-2 bg-white/5 hover:bg-accent text-muted-foreground rounded-lg font-bold text-xs flex items-center gap-2 transition-colors border border-border"
 >
 ...
 <h3 className="font-bold text-foreground font-outfit text-sm flex items-center gap-2 uppercase tracking-wider">
 ...
 <p className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground mb-1">Plano Atual</p>
 <p className="font-bold text-foreground text-sm">{res.conexao.plano}</p>
 </div>
 <div className="bg-card p-4 rounded-xl border border-border ">
 <p className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground mb-1">Uptime</p>
 <p className="font-bold text-foreground text-sm">{res.conexao.uptime}</p>
 </div>
 <div className="bg-card p-4 rounded-xl border border-border ">
 <p className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground mb-1">IP Designado</p>
 <p className="font-mono text-sm font-bold text-foreground">{res.conexao.ip}</p>
 </div>
 <div className="bg-card p-4 rounded-xl border border-border ">
 <p className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground mb-1">Sinal Óptico (ONU)</p>
 ...
 <button className="flex-1 bg-card hover:bg-accent border border-border text-muted-foreground py-3 rounded-xl font-bold transition-all text-sm flex items-center justify-center gap-2">
 ...
 <div>
 <div className="mb-6">
 <h3 className="font-bold text-foreground font-outfit text-lg flex items-center gap-2">
 <ArrowUpRight className="text-blue-400" /> Oportunidades de Upgrade (Cross-sell)
 </h3>
 <p className="text-muted-foreground text-sm">Com base no consumo de {res.metricas.consumo_mes_gb}GB/mês, estes planos são recomendados.</p>
 </div>
 ...
 plano.destaque ? 'border-blue-600 bg-blue-500/10/30' : 'border-border bg-card hover:border-blue-300'
 }`}>
 {plano.destaque && (
 ...
 <div className="flex justify-between items-start mb-2">
 <h4 className="text-2xl font-bold text-foreground font-outfit">{plano.nome}</h4>
 </div>
 <p className="text-3xl font-bold font-outfit text-foreground mb-6">
 R$ {plano.valor.toFixed(2).replace('.', ',')} <span className="text-sm font-medium text-muted-foreground">/mês</span>
 </p>
 ...
 plano.destaque ? 'bg-blue-700 hover:bg-blue-800 text-white -700/20' : 'bg-white/5 hover:bg-accent text-card-foreground border border-border'
 }`}>
