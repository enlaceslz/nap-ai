...
export default function Auditoria() {
 ...
 return (
 <div className="flex-1 flex flex-col h-full bg-background text-card-foreground overflow-y-auto">
 {/* Header Principal */}
 <div className="border-b border-border bg-card/80 backdrop-blur-md px-6 py-5 shrink-0">
 ...
 Rastreamento forense, seguro e imutável de todas as ações executadas por operadores no DJD.
 </p>
 </div>
 </div>
 </div>
 ...
 className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-accent text-muted-foreground border border-border text-xs font-semibold flex items-center gap-1.5 transition-colors"
 >
 <Download size={14} />
 Exportar CSV
 </button>
 ...
 disabled={refreshing}
 className="p-1.5 rounded-lg bg-white/5 hover:bg-accent text-muted-foreground border border-border transition-colors disabled:opacity-50"
 ...
 <p className="text-2xl font-bold text-foreground mt-1.5">{estatisticas.total}</p>
 <span className="text-[10px] text-muted-foreground mt-1 block">Trilha imutável</span>
 </div>
 ...
 <span className="text-[10px] text-muted-foreground mt-1 block">Sessões autenticadas</span>
 </div>
 ...
 <span className="text-[10px] text-muted-foreground mt-1 block">SGP / IXC / Hubsoft</span>
 </div>
 ...
 <span className="text-[10px] text-muted-foreground mt-1 block">Reboots / Wi-Fi CPE</span>
 </div>
 ...
 <span className="text-[10px] text-muted-foreground mt-1 block">Campanhas & Régua</span>
 </div>

 <div className="bg-card border border-rose-500/20 bg-rose-500/5 rounded-xl p-3.5">
 ...
 <span className="text-[11px] font-medium text-muted-foreground">Auditoria Forense Ativa</span>
 </div>
 </div>
 ...
 return (
 <div className="bg-card/95 backdrop-blur-md border border-border rounded-xl p-3 shadow-2xl text-xs">
 <div className="flex items-center gap-2 mb-1">
 <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: data.color }} />
 <span className="font-bold text-foreground">{data.name}</span>
 </div>
 <p className="text-muted-foreground font-mono">
 <span className="text-foreground font-bold">{data.value}</span> {data.value === 1 ? 'registro' : 'registros'} ({data.pct}%)
 </p>
 <p className="text-[10px] text-muted-foreground mt-1">{data.descricao}</p>
 </div>
 );
 }
 return null;
 ...
 <span className="text-3xl font-extrabold text-foreground tracking-tight font-mono">{logs.length}</span>
 <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Ações Totais</span>
 </div>
 </div>
 ...
 : 'bg-background border-border hover:border-white/15 hover:bg-background/80'
 }`}
 >
 ...
 />
 <span className="font-semibold text-xs text-foreground group-hover:text-foreground transition-colors truncate">
 {item.name}
 </span>
 </div>
 <span 
 ...
 <p className="text-[10px] text-muted-foreground line-clamp-1 mb-2.5">
 {item.descricao}
 </p>

 <div className="flex items-center justify-between text-xs pt-2 border-t border-border">
 <span className="font-mono text-card-foreground font-bold">
 {item.value} {item.value === 1 ? 'registro' : 'registros'}
 </span>
 <span className="text-[10px] text-muted-foreground group-hover:text-blue-400 flex items-center gap-1 transition-colors">
 ...
 className="w-full bg-background border border-border rounded-lg pl-9 pr-3 py-2 text-xs text-foreground placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
 />
 </div>
 ...
 className="bg-transparent text-xs text-muted-foreground font-medium focus:outline-none cursor-pointer"
 >
 <option value="todos" className="bg-card">Todos os Módulos</option>
 <option value="Acesso" className="bg-card">Acessos & Logins</option>
 <option value="SGP" className="bg-card">SGP / ERP</option>
 <option value="GenieACS" className="bg-card">GenieACS (TR-069)</option>
 <option value="Zabbix" className="bg-card">NOC / Zabbix</option>
 <option value="WhatsApp" className="bg-card">WhatsApp WABA</option>
 <option value="Campanha" className="bg-card">Campanhas</option>
 <option value="Configura" className="bg-card">Configurações Globais</option>
 </select>
 </div>
 ...
 className="bg-transparent text-xs text-muted-foreground font-medium focus:outline-none cursor-pointer"
 >
 <option value="todas" className="bg-card">Todas Severidades</option>
 <option value="info" className="bg-card">Informativo</option>
 <option value="atencao" className="bg-card">Atenção</option>
 <option value="critico" className="bg-card">Crítico</option>
 </select>
 </div>
 ...
 className="bg-transparent text-xs text-muted-foreground font-medium focus:outline-none cursor-pointer"
 >
 <option value="todas" className="bg-card">Todos os Tipos</option>
 <option value="acesso" className="bg-card">Acessos</option>
 <option value="configuracao" className="bg-card">Configurações</option>
 <option value="comando" className="bg-card">Comandos</option>
 <option value="disparo" className="bg-card">Disparos</option>
 </select>
 </div>
 ...
 <thead>
 <tr className="border-b border-border bg-white/[0.02] text-muted-foreground font-semibold uppercase tracking-wider text-[11px]">
 ...
 <div>
 <p className="font-semibold text-foreground truncate max-w-[140px]">{log.usuario}</p>
 {log.usuarioEmail && (
 <p className="text-[10px] text-muted-foreground truncate max-w-[140px]">{log.usuarioEmail}</p>
 )}
 </div>
 </div>
 </td>
 ...
 <td className="py-3 px-4 whitespace-nowrap">
 <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-white/5 border border-border text-muted-foreground text-[11px] font-medium">
 ...
 <td className="py-3 px-4">
 <p className="font-semibold text-card-foreground">{log.acao}</p>
 <p className="text-[11px] text-muted-foreground line-clamp-1 max-w-xs">{log.detalhes}</p>
 </td>
 ...
 <button
 onClick={() => setSelectedLog(log)}
 className="p-1.5 rounded-lg bg-white/5 hover:bg-accent text-muted-foreground hover:text-foreground transition-colors"
 ...
 <div className="p-3.5 border-t border-border bg-white/[0.01] flex items-center justify-between text-xs text-muted-foreground">
 ...
 <div className="bg-card border border-border rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
 {/* Modal Header */}
 ...
 <div>
 <h3 className="font-bold text-foreground text-base">Evidência Forense de Auditoria</h3>
 <p className="text-xs text-muted-foreground font-mono">ID: {selectedLog.id}</p>
 </div>
 </div>
 <button
 ...
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block">Severidade</span>
 <div className="mt-1">{getSeveridadeBadge(selectedLog.severidade)}</div>
 </div>

 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block">Módulo</span>
 ...
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block">IP de Origem</span>
 <span className="text-xs font-mono font-semibold text-muted-foreground mt-1 block">
 {selectedLog.ip}
 </span>
 </div>
 </div>
 ...
 <div>
 <p className="text-sm font-bold text-foreground">{selectedLog.usuario}</p>
 ...
 <p className="text-sm font-semibold text-foreground">{selectedLog.acao}</p>
 <p className="text-xs text-muted-foreground mt-2 leading-relaxed bg-card p-3 rounded-lg border border-border">
 ...
 <pre className="text-[11px] font-mono text-muted-foreground bg-card p-2.5 rounded-lg border border-border overflow-x-auto max-h-40">
 ...
 <h3 className="font-bold text-foreground text-sm">Registrar Evento de Auditoria</h3>
 </div>
 <button 
 ...
 <div>
 <label className="text-xs font-medium text-muted-foreground block mb-1">Ação Executada</label>
 <input
 type="text"
 ...
 <div>
 <label className="text-xs font-medium text-muted-foreground block mb-1">Detalhes da Ação</label>
 <textarea
 ...
 className="w-full bg-background border border-border rounded-lg p-2 text-xs text-foreground focus:outline-none focus:border-blue-500 h-20 resize-none"
 required
 />
 </div>
 ...
 <div>
 <label className="text-xs font-medium text-muted-foreground block mb-1">Severidade</label>
 <select
