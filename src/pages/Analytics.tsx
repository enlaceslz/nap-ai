...
export default function Analytics() {
 ...
 <div>
 <h1 className="text-3xl font-bold text-foreground font-outfit mb-2">Visão Geral da Operação</h1>
 <p className="text-muted-foreground">Monitoramento e inteligência analítica do ecossistema DJD Telecom.</p>
 {toastMsg && (
 ...
 <h3 className="text-lg font-bold text-foreground font-outfit mb-6 flex items-center gap-2">
 ...
 <h3 className="text-lg font-bold text-foreground font-outfit mb-6">Tempo Médio de Resposta (s)</h3>
 <div className="h-72">
 <ResponsiveContainer width="100%" height="100%">
 ...
 <div>
 <h3 className="text-lg font-bold text-foreground font-outfit">Volume de Resolução: Humano vs IA</h3>
 <p className="text-xs text-muted-foreground mt-1">Comparativo de tickets encerrados sem intervenção humana na última semana.</p>
 </div>
 <button className="text-xs bg-background border border-border text-muted-foreground px-3 py-1.5 rounded-lg hover:bg-accent transition-colors">
 Exportar CSV
 </button>
 </div>
 <div className="h-80">
 ...
 />
 <Legend iconType="circle" wrapperStyle={{ fontSize: '12px', paddingTop: '10px' }} />
 <Area type="monotone" dataKey="ia" name="DJD IA Integrada" stroke="#6366f1" strokeWidth={3} fillOpacity={1} fill="url(#colorIa)" />
 ...
 <div>
 <h3 className="text-lg font-bold text-foreground font-outfit">Operadores Online & Filas Asterisk 20+</h3>
 <p className="text-xs text-muted-foreground mt-0.5">Ramais SIP ativos no Asterisk 21 e distribuição de canais</p>
 </div>
 ...
 <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-muted-foreground font-bold border border-border">
 {op.nome.split(' ').map(n => n[0]).slice(0, 2).join('')}
 </div>
 <div>
 <div className="flex items-center gap-2">
 <p className="text-foreground font-semibold text-sm">{op.nome}</p>
 <span className="text-[10px] font-mono text-muted-foreground bg-white/5 px-1.5 py-0.5 rounded border border-border">
 Ramal {op.ramal}
 </span>
 </div>
 <p className="text-xs text-muted-foreground">{op.fila}</p>
 </div>
 </div>
 ...
 <div className="p-4 bg-background border-t border-border flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground font-mono">
 <div className="flex items-center gap-2">
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
 <div className="bg-card border border-emerald-500/20 p-6 rounded-2xl relative overflow-hidden group">
 ...
 <h4 className="text-muted-foreground text-xs font-bold uppercase tracking-wider mb-1">NPS Score Global</h4>
 <div className="flex items-baseline gap-2">
 <span className="text-4xl font-bold text-foreground font-outfit">+{npsStats?.npsScore || 78}</span>
 ...
 <p className="text-xs text-muted-foreground mt-0.5">Baseado em {npsStats?.totalRespostas || 486} avaliações validadas nos últimos 30 dias</p>
 </div>
 ...
 {/* Barra Segmentada */}
 <div className="h-5 w-full bg-muted rounded-full overflow-hidden flex shadow-inner">
 <div 
 ...
 <div>
 <p className="text-xs text-muted-foreground">Promotores (Notas 9-10)</p>
 <p className="text-lg font-bold text-foreground font-outfit">408 clientes ({npsStats?.promotoresPct || 84}%)</p>
 ...
 <div>
 <p className="text-xs text-muted-foreground">Passivos / Neutros (Notas 7-8)</p>
 <p className="text-lg font-bold text-foreground font-outfit">54 clientes ({npsStats?.neutrosPct || 11}%)</p>
 ...
 <div>
 <p className="text-xs text-muted-foreground">Detratores (Notas 0-6)</p>
 <p className="text-lg font-bold text-foreground font-outfit">24 clientes ({npsStats?.detratoresPct || 5}%)</p>
 ...
 <div className="lg:col-span-2 bg-card border border-border rounded-2xl p-6">
 <h3 className="text-base font-bold text-foreground font-outfit mb-4 flex items-center gap-2">
 ...
 <div>
 <h3 className="text-base font-bold text-foreground font-outfit mb-2 flex items-center gap-2">
 <Zap size={18} className="text-amber-400" />
 Gatilho Automático de Pesquisa
 </h3>
 <p className="text-xs text-muted-foreground leading-relaxed mb-4">
 O DJD monitora o encerramento de tickets no Inbox, Webchat e URA. Após o prazo configurado, uma mensagem interativa de avaliação é despachada.
 </p>
 ...
 <p className="text-xs text-muted-foreground mt-0.5">Comentários e notas registradas pelos clientes em todos os canais</p>
 </div>
 ...
 className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${filtroNps === 'todos' ? 'bg-white/10 text-foreground font-bold' : 'text-muted-foreground hover:text-foreground'}`}
 >
 Todos ({npsFeed.length})
 </button>
 <button
 onClick={() => setFiltroNps('promotor')}
 className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${filtroNps === 'promotor' ? 'bg-emerald-500/20 text-emerald-400 font-bold' : 'text-muted-foreground hover:text-white'}`}
 >
 Promotores
 </button>
 <button
 onClick={() => setFiltroNps('neutro')}
 className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${filtroNps === 'neutro' ? 'bg-amber-500/20 text-amber-400 font-bold' : 'text-muted-foreground hover:text-white'}`}
 >
 Neutros
 </button>
 <button
 onClick={() => setFiltroNps('detrator')}
 className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${filtroNps === 'detrator' ? 'bg-rose-500/20 text-rose-400 font-bold' : 'text-muted-foreground hover:text-white'}`}
 >
 Detratores
 </button>
 </div>
 ...
 <div 
 key={item.id}
 onClick={() => showToast('Detalhes do feedback do cliente')} className="p-4 rounded-xl bg-background border border-border hover:border-border transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-pointer"
 >
 ...
 <span className="text-[11px] text-muted-foreground font-mono">{item.telefone}</span>
 ...
 <div className="flex sm:flex-col items-end justify-between sm:justify-center text-right shrink-0 border-t sm:border-t-0 pt-2 sm:pt-0 border-border">
 <span className="text-xs text-muted-foreground font-medium">Atendente: <strong className="text-foreground">{item.atendente}</strong></span>
 ...
 <h2 className="text-xl font-bold text-foreground font-outfit mb-4 flex items-center gap-2"><Radio className="text-purple-500 animate-pulse" /> Radar NOC (C.C.O)</h2>
 <p className="text-muted-foreground text-sm mb-6">Acompanhamento em tempo real das viaturas em campo via GPS (PWA Técnico), status do Asterisk PABX e contenção da IA Gemini.</p>
 
 <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
 {/* COL 1: Grid Map */}
 <div className="lg:col-span-2 bg-background border border-border rounded-xl p-4 relative overflow-hidden">
 <div className="flex justify-between items-center mb-4 relative z-10">
 <h3 className="text-sm font-bold text-muted-foreground flex items-center gap-2"><MapPin size={16} className="text-emerald-400" /> Viaturas em Campo (GPS)</h3>
 ...
 <div className="relative w-full h-[400px] border border-border rounded-xl overflow-hidden bg-card shadow-inner">
 {mapaTecnicos ? (
 <MapContainer 
 ...
 <Popup className="bg-card border border-border rounded-xl">
 <div className="p-1">
 <p className="font-bold text-foreground text-xs">{tec.nome}</p>
 <p className="text-[10px] text-emerald-400 mt-1">{tec.status_label}</p>
 <p className="text-[10px] text-muted-foreground font-mono mt-1">{tec.velocidade_kmh} km/h • Bat: {tec.bateria}%</p>
 </div>
 </Popup>
 </Marker>
 );
 ...
 <Popup>
 <div className="p-1">
 <p className="font-bold text-foreground text-xs">{os.numero}</p>
 <p className="text-[10px] text-muted-foreground mt-1">{os.cliente_nome}</p>
 <p className="text-[10px] text-muted-foreground mt-1">{os.observacoes}</p>
 </div>
 </Popup>
 </Marker>
 );
 ...
 <h3 className="text-sm font-bold text-muted-foreground mb-4 flex items-center gap-2"><Phone size={16} className="text-blue-400" /> PABX (Asterisk/WebRTC)</h3>
 <div className="space-y-3 relative z-10">
 <div className="flex justify-between items-center border-b border-border pb-2">
 <span className="text-xs text-muted-foreground">Chamadas Ativas</span>
 <span className="text-sm font-bold text-emerald-400">12</span>
 </div>
 <div className="flex justify-between items-center border-b border-border pb-2">
 <span className="text-xs text-muted-foreground">Fila de Espera</span>
 <span className="text-sm font-bold text-orange-400">3</span>
 </div>
 <div className="flex justify-between items-center border-b border-border pb-2">
 <span className="text-xs text-muted-foreground">Tempo Médio Espera (TME)</span>
 <span className="text-sm font-bold text-foreground">01m 45s</span>
 </div>
 </div>
 </div>
 ...
 <h3 className="text-sm font-bold text-muted-foreground mb-4 flex items-center gap-2"><Bot size={16} className="text-indigo-400" /> Cérebro IA (Gemini)</h3>
 <div className="space-y-3 relative z-10">
 <div className="flex justify-between items-center border-b border-border pb-2">
 <span className="text-xs text-muted-foreground">Resolução Contida (FCR)</span>
 <span className="text-sm font-bold text-emerald-400">72%</span>
 </div>
 <div className="flex justify-between items-center border-b border-border pb-2">
 <span className="text-xs text-muted-foreground">Transferências (Handoff)</span>
 <span className="text-sm font-bold text-orange-400">28%</span>
 </div>
 <div className="flex justify-between items-center border-b border-border pb-2">
 <span className="text-xs text-muted-foreground">Erros de Compreensão</span>
...
function MetricCard({ title, value, trend, trendUp, icon }: any) {
 return (
 <div className="bg-card border border-border p-6 rounded-2xl flex flex-col relative overflow-hidden group">
 <div className="absolute -right-6 -top-6 text-card-foreground/30 group-hover:text-muted-foreground/30 transition-colors duration-500 rotate-12 scale-150">
 {icon}
 </div>
 ...
 <span className="text-3xl font-bold text-foreground font-outfit">{value}</span>
 </div>
 </div>
 );
}
