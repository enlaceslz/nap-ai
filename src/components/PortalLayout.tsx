...
export default function PortalLayout() {
 ...
 <div className="w-20 h-20 rounded-2xl bg-card shadow-xl flex items-center justify-center mb-6 border border-slate-100">
 <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#0a50ff] to-[#55b0ff] shadow-inner flex items-center justify-center">
 <span className="text-foreground font-black text-2xl font-sans mt-[2px]">{inicialProvedor}</span>
 </div>
 </div>
 
 <h1 className="text-2xl font-bold text-foreground mb-2 font-outfit">Que saudade, {clientName.split(' ')[0]}!</h1>
 <p className="text-muted-foreground text-sm mb-8 leading-relaxed">
 Notamos que o seu contrato <strong className="text-muted-foreground">{clientContrato}</strong> está inativo. 
 ...
 <div className="bg-card rounded-3xl border border-blue-100 p-6 w-full shadow-lg shadow-blue-900/5 mb-6 relative overflow-hidden">
 ...
 <h3 className="text-lg font-bold text-foreground mb-1">Fibra Turbo + Dobro de Mega</h3>
 <p className="text-muted-foreground text-xs mb-4">Assine hoje e ganhe instalação grátis e roteador Wi-Fi 6 incluso.</p>
 ...
 localStorage.removeItem('@nap_client_auth');
 window.location.href = '/portal/login';
 }} className="text-xs font-bold text-muted-foreground hover:text-muted-foreground">
 ...
 return (
 <div className="flex flex-col md:flex-row h-screen bg-background text-muted-foreground font-sans">
 {/* Mobile Header */}
 <div className="md:hidden bg-white/90 backdrop-blur-md border-b border-border p-4 flex justify-between items-center z-10 sticky top-0">
 ...
 <span className="font-bold text-lg text-foreground font-outfit truncate max-w-[170px]">{nomeProvedor}</span>
 </div>
 <div className="flex items-center gap-3">
 {isSupported && permission !== 'granted' && (
 <button onClick={requestPermission} className="p-2 text-muted-foreground hover:text-blue-600 transition-colors" title="Ativar Notificações">
 <Bell size={20} />
 </button>
 )}
 {permission === 'granted' && (
 ...
 <PWAInstallButton />
 <div className="w-9 h-9 rounded-full bg-slate-100 border border-border flex items-center justify-center font-bold text-sm text-muted-foreground">
 {clientInitials}
 </div>
 </div>
 </div>
 ...
 {/* Sidebar Navigation */}
 <aside className="hidden md:flex w-64 bg-card border-r border-border flex-col z-10">
 ...
 <span className="font-bold text-base text-foreground font-outfit leading-tight truncate max-w-[160px]">{nomeProvedor}</span>
 ...
 className="flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2 text-sm font-bold text-muted-foreground hover:bg-background transition-all active:scale-95"
 >
 <Bell size={18} />
 Ativar Notificações
 </button>
 ...
 <p className="text-foreground font-bold text-xs truncate" title={clientName}>{clientName}</p>
 <p className="text-[10px] text-muted-foreground font-mono truncate">{clientContrato}</p>
 </div>
 </div>
 <button
 onClick={() => {
 ...
 className="text-[10px] font-bold text-muted-foreground hover:text-red-600 transition-colors px-2 py-1 rounded hover:bg-red-50 shrink-0"
 ...
 <nav className="md:hidden bg-white/95 backdrop-blur-xl border-t border-border flex justify-around p-2 pb-safe [0_-5px_15px_-5px_rgba(0,0,0,0.1)] z-10 sticky bottom-0">
...
function NavItem({ to, icon, label, exact = false }: { to: string; icon: React.ReactNode; label: string; exact?: boolean }) {
 ...
 {({ isActive }) => (
 <>
 <div className={`${isActive ? 'text-blue-600' : 'text-muted-foreground group-hover:text-muted-foreground'} transition-colors`}>
 {icon}
 </div>
 <span>{label}</span>
 </>
