...
export default function ErpPingBadge({
 ...
 if (loading) {
 return (
 <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-white/5 border border-border text-muted-foreground text-[10px] font-mono">
 ...
 if (!ping) {
 return (
 <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-white/5 text-muted-foreground text-[10px] font-mono">
 <Radio size={10} />
 <span>-- ms</span>
 </span>
 );
 ...
 <span className={`w-0.5 rounded-full transition-all ${level >= 1 ? (level === 1 ? 'bg-amber-400 h-1.5' : 'bg-emerald-400 h-1.5') : 'bg-accent h-1.5'}`} />
 <span className={`w-0.5 rounded-full transition-all ${level >= 2 ? (level <= 2 ? 'bg-amber-400 h-2' : 'bg-emerald-400 h-2') : 'bg-accent h-2'}`} />
 <span className={`w-0.5 rounded-full transition-all ${level >= 3 ? 'bg-emerald-400 h-2.5' : 'bg-accent h-2.5'}`} />
 <span className={`w-0.5 rounded-full transition-all ${level >= 4 ? 'bg-emerald-400 h-3' : 'bg-accent h-3'}`} />
 </span>
 );
 };
