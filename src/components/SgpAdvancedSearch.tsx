...
export default function SgpAdvancedSearch({
  ...
  return (
    <div className={`bg-slate-900 rounded-2xl border border-white/10  overflow-hidden flex flex-col ${className}`}>
      
      {/* Header com Barra de Consulta Rápida SGP */}
      <div className="p-4 sm:p-5 border-b border-white/10 bg-slate-950/80">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
          ...
          {cliente && (
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="font-mono font-bold text-slate-200 bg-slate-900 px-2 py-0.5 rounded border border-white/10">
                #{cliente.contrato_id}
              </span>
            </div>
          )}
        ...
        <form onSubmit={handleSearchSubmit} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-9 pr-3 py-2 bg-slate-900 border border-white/10 rounded-xl text-xs sm:text-sm text-slate-200 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-600/40 focus:border-blue-600  font-medium"
            />
          </div>
          <button
            type="submit"
      ...
      {cliente ? (
        <div className="p-4 sm:p-5 flex-1 overflow-y-auto space-y-4">
          ...
          <div className="bg-slate-950 border border-white/10 rounded-xl p-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className="font-bold text-white text-sm">{cliente.nome}</h4>
              ...
              <div className="bg-slate-900 px-3.5 py-2.5 rounded-xl border border-white/10 flex items-center gap-4 text-xs ">
                <div>
            ...
            <div className="mt-3 pt-3 border-t border-white/10/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
          ...
          <div className="flex border-b border-white/10 gap-1 text-xs">
            <button
          ...
          {activeTab === 'financeiro' && (
            <div className="space-y-3">
              ...
              {cliente.faturas && cliente.faturas.length > 0 ? (
                <div className="divide-y divide-white/5 border border-white/10 rounded-xl overflow-hidden bg-slate-900">
                  {cliente.faturas.map((fatura) => (
                    <div key={fatura.id} className="p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-950/70 transition-colors">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            Vencimento: <span className="font-semibold text-slate-300">{fatura.vencimento}</span> • Valor: <span className="font-bold text-white">R$ {fatura.valor.toFixed(2)}</span>
                            {fatura.pago_em && (
                          ...
                          {fatura.status !== 'pago' && fatura.pix_copia_cola && (
                            <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px]">
                              <button
                                ...
                                className="flex items-center gap-1 text-slate-400 hover:text-blue-600 bg-slate-800 hover:bg-blue-500/10 px-2 py-1 rounded border border-white/10 transition-colors"
                              ...
                              {fatura.link_pdf && (
                                <a 
                                  ...
                                  target="_blank" 
                                  rel="noreferrer"
                                  className="flex items-center gap-1 text-slate-400 hover:text-white bg-slate-800 px-2 py-1 rounded border border-white/10"
                                >
                                  <Download size={12} />
                ...
                <div className="p-8 text-center text-slate-400 text-xs bg-slate-950 rounded-xl border border-white/10">
          ...
          {activeTab === 'conexao' && cliente.conexao && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
              ...
              <div className="p-3 bg-slate-950 rounded-xl border border-white/10 col-span-2 sm:col-span-2">
          ...
          {activeTab === 'contrato' && (
            <div className="space-y-4 text-xs">
              <div className="p-4 bg-slate-900 rounded-2xl border border-white/10 space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/5 pb-3">
                  <div>
                    <span className="text-[10px] uppercase font-bold text-slate-400 block">Instrumento Contratual SCM</span>
                    <span className="text-base font-bold text-white font-mono">CTR-2026-8894</span>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="p-2.5 bg-slate-950 rounded-xl border border-white/5">
                    <span className="text-[10px] text-slate-400 font-bold block">EQUIPAMENTO COMODATO</span>
                    <span className="font-bold text-slate-200">ONT Wi-Fi 6 Gigabit (ZTE)</span>
                    <span className="text-[10px] text-slate-500 block font-mono">MAC: 48:D2:24:F8:91:A0</span>
                  </div>
                  <div className="p-2.5 bg-slate-950 rounded-xl border border-white/5">
                    <span className="text-[10px] text-slate-400 font-bold block">DATA DE ADESÃO</span>
                    <span className="font-bold text-slate-200">12/09/2026 10:14</span>
                    <span className="text-[10px] text-slate-500 block">IP: 177.67.240.12</span>
                  </div>
                  <div className="p-2.5 bg-slate-950 rounded-xl border border-white/5">
                    <span className="text-[10px] text-slate-400 font-bold block">CLÁUSULA FIDELIDADE</span>
                    <span className="font-bold text-slate-200">12 Meses (Isenção)</span>
      ...
      {ofertaModalOpen && selectedPlanoParaOferta && cliente && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-slate-900 rounded-2xl border border-white/10  max-w-md w-full p-5 sm:p-6 space-y-4">
            ...
            <div className="p-3.5 bg-slate-950 border border-white/10 rounded-xl space-y-2 text-xs">
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-semibold">Cliente:</span>
                <span className="font-bold text-slate-200">{cliente.nome}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-semibold">Plano Atual:</span>
                <span className="text-slate-400 font-mono">{cliente.plano_atual?.nome} (R$ {cliente.plano_atual?.valor?.toFixed(2)})</span>
              </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full p-2.5 bg-slate-900 border border-white/10 rounded-xl text-xs text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-600/40 focus:border-blue-600"
            ...
            <div className="flex gap-2 pt-2">
              <button
                type="button"
                onClick={() => setOfertaModalOpen(false)}
                className="flex-1 py-2.5 bg-slate-800 hover:bg-slate-200 text-slate-300 rounded-xl text-xs font-bold transition-colors"
              >
                Cancelar
              </button>
              <button
