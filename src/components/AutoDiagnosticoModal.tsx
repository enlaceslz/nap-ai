...
export default function AutoDiagnosticoModal({ isOpen, onClose, clienteBairro = 'Centro Histórico' }: AutoDiagnosticoModalProps) {
 ...
 <div className="bg-card rounded-3xl w-full max-w-lg p-6 md:p-8 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
 ...
 <div>
 <h3 className="text-xl font-bold text-foreground font-outfit">Auto-Diagnóstico de Rede</h3>
 <p className="text-xs text-muted-foreground">Telemetria GenieACS & Varredura NOC em tempo real</p>
 </div>
 </div>
 <button 
 onClick={onClose} 
 className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-muted-foreground flex items-center justify-center font-bold transition-colors"
 >
 ✕
 </button>
 </div>
 ...
 : 'bg-card border-slate-100 opacity-60'
 }`}
 >
 ...
 {step.status === 'pending' && <span className="w-3.5 h-3.5 rounded-full border-2 border-border inline-block"></span>}
 {step.label}
 </span>
 ...
 {step.status === 'pending' && <span className="text-muted-foreground">Aguardando</span>}
 </span>
 </div>
 {step.detalhe && (
 <p className="text-[11px] text-muted-foreground mt-1.5 pl-5 font-mono">
 {step.detalhe}
 </p>
 )}
 </div>
 ...
 <h4 className="font-bold text-foreground text-sm">Reiniciando seu Roteador remotamente...</h4>
 <p className="text-xs text-muted-foreground">Comando TR-069 transmitido com sucesso. Aguarde o ciclo de energia.</p>
 ...
 className="flex-1 py-3 px-4 rounded-xl border border-border hover:bg-background text-muted-foreground font-bold text-xs transition-all flex items-center justify-center gap-2 disabled:opacity-50"
 >
