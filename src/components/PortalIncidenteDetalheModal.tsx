...
export default function PortalIncidenteDetalheModal({
 ...
 <div className="bg-card border border-amber-500/20 rounded-3xl w-full max-w-lg p-6 shadow-2xl relative overflow-hidden flex flex-col">
 {/* Glow de aviso */}
 ...
 <h3 className="font-bold text-foreground text-base font-outfit">{incidente.titulo}</h3>
 </div>
 </div>
 <button 
 onClick={onClose}
 ...
 <span className="text-[10px] text-muted-foreground block">Previsão de Normalização:</span>
 <span className="font-bold font-mono text-foreground text-sm">{incidente.previsaoRetorno}</span>
 </div>
 </div>
 ...
 <div>
 <span className="text-foreground font-bold block text-[11px]">Ruptura Identificada por OTDR</span>
 <span className="text-[10px] text-muted-foreground">Telemetria óptica apontou atenuação a 4.1km do POP</span>
 </div>
 </div>
 ...
 <span className="text-[10px] text-muted-foreground">Caixa de emenda aberta, cabos sendo re-fundidos</span>
 </div>
 </div>

 <div className="flex items-start gap-3 relative">
 <div className="w-4 h-4 rounded-full bg-accent flex items-center justify-center text-[10px] font-bold text-muted-foreground shrink-0 mt-0.5 z-10">
 ○
 </div>
 <div>
 <span className="text-muted-foreground font-medium block text-[11px]">Sincronização com Concentrador BNG</span>
 <span className="text-[10px] text-muted-foreground">Previsão: {incidente.previsaoRetorno}</span>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="pt-4 mt-4 border-t border-border flex flex-col gap-3 relative z-10">
 {feedback && (
 ...
 : 'bg-white/5 hover:bg-accent text-muted-foreground hover:text-foreground border border-border'
 }`}
 >
