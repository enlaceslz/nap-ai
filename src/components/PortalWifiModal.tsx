...
export default function PortalWifiModal({ isOpen, onClose, onSuccess }: PortalWifiModalProps) {
 ...
 if (!pwd) return { label: 'Vazia', score: 0, color: 'bg-slate-200 text-muted-foreground' };
 ...
 default: return <Radio size={16} className="text-muted-foreground" />;
 }
 };
 ...
 <div 
 className="bg-card w-full max-w-2xl rounded-3xl shadow-2xl border border-border flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200"
 role="dialog"
 aria-modal="true"
 >
 {/* Header do Modal */}
 <div className="p-5 md:p-6 bg-gradient-to-r from-blue-700 via-indigo-800 to-slate-900 text-foreground flex items-center justify-between relative overflow-hidden shrink-0">
 ...
 <div className="w-12 h-12 rounded-2xl bg-white/10 border border-border backdrop-blur-md flex items-center justify-center text-foreground shadow-inner">
 <Wifi size={24} />
 </div>
 <div>
 ...
 <button 
 onClick={onClose}
 className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 text-foreground/80 hover:text-foreground flex items-center justify-center transition-all relative z-10"
 aria-label="Fechar"
 >
 <X size={18} />
 </button>
 ...
 {/* Abas de Navegação */}
 <div className="px-6 pt-3 pb-0 bg-background border-b border-border flex items-center justify-between gap-2 shrink-0">
 ...
 activeTab === 'alterar'
 ? 'border-blue-600 text-blue-700 bg-card shadow-sm'
 ...
 <div className="p-4 bg-background rounded-2xl border border-border flex flex-col sm:flex-row justify-between sm:items-center gap-4">
 <div className="space-y-1">
 <div className="flex items-center gap-2">
 <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Nome da Rede (SSID):</span>
 ...
 <span className="text-[10px] uppercase font-bold text-muted-foreground">Senha Atual</span>
 ...
 className="p-1.5 text-muted-foreground hover:text-muted-foreground hover:bg-slate-100 rounded-lg transition-colors"
 ...
 className="p-1.5 text-muted-foreground hover:text-blue-600 hover:bg-slate-100 rounded-lg transition-colors"
 title="Copiar senha"
 >
 ...
 className="w-full pl-4 pr-11 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-blue-600/50 focus:border-blue-600 outline-none text-foreground font-mono text-sm transition-all"
 required
 minLength={8}
 maxLength={63}
 />
 ...
 className="w-full px-4 py-3 bg-background border border-border rounded-xl focus:ring-2 focus:ring-blue-600/50 focus:border-blue-600 outline-none text-foreground font-mono text-sm transition-all"
 required
 minLength={8}
 maxLength={63}
 />
 ...
 className="w-full sm:w-auto px-4 py-2.5 text-xs font-bold text-muted-foreground hover:text-red-600 hover:bg-red-50 rounded-xl border border-border transition-colors flex items-center justify-center gap-1.5"
 >
 ...
 type="button"
 onClick={onClose}
 className="flex-1 sm:flex-initial px-5 py-2.5 border border-border hover:bg-slate-100 text-muted-foreground rounded-xl text-xs font-bold transition-all"
 >
 Cancelar
 </button>
 <button
 ...
 <div>
 <h4 className="text-sm font-bold text-foreground">Aparelhos Conectados no Roteador</h4>
 <p className="text-xs text-muted-foreground">Mapeamento em tempo real via tabela ARP e TR-069.</p>
 </div>
 ...
 <div className="divide-y divide-slate-100 border border-border rounded-2xl overflow-hidden bg-card">
 {wifiData?.dispositivosConectados.map((dev, idx) => (
 <div key={idx} className="p-3.5 flex items-center justify-between hover:bg-background transition-colors">
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center">
 {getDeviceIcon(dev.tipo)}
 </div>
 <div>
 <p className="text-xs font-bold text-foreground">{dev.nome}</p>
 ...
 <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Sinal</span>
 ...
 <div className="p-6 bg-card rounded-3xl border-2 border-dashed border-blue-200 shadow-sm flex flex-col items-center">
 <div className="w-48 h-48 bg-card rounded-2xl p-4 flex flex-col items-center justify-center text-foreground relative shadow-lg">
 ...
 <p className="text-xs font-bold text-foreground mt-4 font-mono">
 ...
 <div className="w-full max-w-sm flex items-center justify-between p-3 bg-background rounded-xl border border-border">
 <div className="text-left">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block">Senha da Rede</span>
 <span className="text-xs font-mono font-bold text-foreground">{wifiData?.senhaWifi}</span>
 </div>
 <button
 type="button"
 onClick={handleCopyPassword}
