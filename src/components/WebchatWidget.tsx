...
export default function WebchatWidget() {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-0 md:bottom-6 right-0 md:right-6 w-full md:w-[390px] h-[90vh] md:h-[620px] bg-[#efeae2] md:rounded-2xl border border-slate-200 shadow-2xl flex flex-col z-50 overflow-hidden transition-all">
          {/* Header */}
          <div className={`p-4 flex justify-between items-center shrink-0 transition-colors ${
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center shrink-0">
                {isHandoff ? <Headphones size={22} className="text-white" /> : <Bot size={22} className="text-white" />}
              </div>
              <div>
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 relative z-0">
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-white rounded-xl rounded-tl-none p-3 shadow-sm flex gap-1.5 items-center">
          ...
          <div className="px-3 py-1.5 bg-[#f0f2f5] border-t border-slate-200/80 flex items-center gap-1.5 overflow-x-auto no-scrollbar shrink-0">
            {!isHandoff && (
              <button 
          ...
          <form onSubmit={handleSend} className="p-3 bg-[#f0f2f5] shrink-0">
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 bg-white border border-slate-300 focus:border-emerald-500 outline-none rounded-full px-4 py-2.5 text-[13.5px] text-[#111b21] placeholder:text-[#8696a0] transition-all shadow-2xs"
              />
              <button 
                type="submit"
                disabled={!inputValue.trim() || isTyping}
                className={`w-10 h-10 rounded-full flex items-center justify-center text-white disabled:opacity-50 transition-all shrink-0 shadow-sm ${
