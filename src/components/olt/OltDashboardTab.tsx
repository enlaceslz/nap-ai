    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div
          ...
          className="cursor-pointer bg-slate-900/80 p-4 rounded-xl border border-slate-800 hover:border-slate-700 transition-all hover:bg-slate-850 group"
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-slate-400">OLTs em Produção</span>
            <div className="p-2 rounded-lg bg-cyan-500/10 text-cyan-400">
              <Server size={18} />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">{metrics?.olts.total ?? olts.length}</span>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-slate-400">ONUs / Assinantes Conectados</span>
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-400">de {metrics?.onus.total ?? 0} totais</span>
          </div>
          <div className="mt-2 flex items-center justify-between text-[11px] text-slate-500">
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-slate-400">ONUs Descobertas (Pendentes)</span>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-slate-400">Alarmes Ópticos (NOC)</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900/80 rounded-xl border border-slate-800 p-5 space-y-4">
          ...
          <div className="space-y-3">
            {olts.map((olt) => (
              <div
                key={olt.id}
                className="p-4 rounded-lg bg-slate-800/40 border border-slate-800 hover:border-slate-700 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-sm text-white">{olt.nome}</span>
                    <span
                  ...
                  <div className="text-xs text-slate-400 flex flex-wrap items-center gap-3">
                    <span>IP: <strong className="text-slate-300">{olt.ip}:{olt.porta}</strong></span>
                    <span>•</span>
                    <span>POP: <strong className="text-slate-300">{olt.pop || 'Principal'}</strong></span>
                    <span>•</span>
                    <span>Firmware: <strong className="text-slate-300">{olt.versao_firmware || 'N/A'}</strong></span>
                  </div>
                </div>

                <div className="flex items-center gap-4 text-xs">
                  <div className="flex items-center gap-1 text-slate-400" title="Uso de Processador">
                    <Cpu size={14} className="text-slate-400" />
                    <span>{olt.cpu_usage ?? 20}%</span>
                  </div>
                  <div className="flex items-center gap-1 text-slate-400" title="Temperatura Térmica">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[10px] text-slate-500">{olt.total_pons ?? 0} portas PON</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-slate-900/80 rounded-xl border border-slate-800 p-5 space-y-4 flex flex-col justify-between">
            ...
            <div className="space-y-2">
              {unassigned.length === 0 ? (
                ...
                unassigned.slice(0, 3).map((u) => (
                  <div
                    key={u.id}
                    className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/60 flex items-center justify-between gap-2"
                  >
                    <div>
                      <div className="font-mono text-xs font-bold text-white">{u.serial}</div>
          ...
          {unassigned.length > 3 && (
            <button
              onClick={() => onSelectTab('unassigned')}
              className="mt-3 w-full py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium text-center transition-colors"
            >
