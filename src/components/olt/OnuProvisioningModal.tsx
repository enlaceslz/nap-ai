...
export const OnuProvisioningModal: React.FC<OnuProvisioningModalProps> = ({
 ...
 <div className="bg-card border border-border rounded-2xl w-full max-w-2xl p-6 space-y-6 shadow-2xl my-8">
 ...
 : 'bg-background/80 hover:bg-muted text-card-foreground border-border'
 }`}
 >
 <span>{u.serial}</span>
 ...
 {cliOutput && (
 <div className="p-3 rounded-lg bg-background border border-border font-mono text-[11px] text-muted-foreground space-y-1">
 <div className="flex items-center gap-1.5 text-cyan-400 font-bold mb-1">
 <Terminal size={13} />
 <span>Log de Execução CLI na OLT:</span>
 </div>
 <pre className="whitespace-pre-wrap text-muted-foreground">{cliOutput}</pre>
 </div>
 )}
 ...
 className="w-full px-3 py-2 rounded-lg bg-background border border-border text-xs text-card-foreground focus:outline-none focus:border-cyan-500 font-mono"
 >
 {pons.map((pon) => (
 ...
 className="w-full px-3 py-2 rounded-lg bg-background border border-border text-xs text-card-foreground font-mono focus:outline-none focus:border-cyan-500 uppercase"
 required
 />
 </div>
 ...
 onClick={onClose}
 disabled={submitting}
 className="px-4 py-2 rounded-lg bg-muted hover:bg-accent text-muted-foreground text-xs font-semibold transition-colors"
 >
 Cancelar
 </button>
 <button
