...
export const OltDevicesTab: React.FC<OltDevicesTabProps> = ({
 ...
 isSelected
 ? 'bg-card border-cyan-500/60 ring-1 ring-cyan-500/20 shadow-lg'
 : 'bg-card/60 border-border hover:border-border hover:bg-card/90'
 }`}
 >
 <div className="flex items-start justify-between gap-2 mb-3">
 <div>
 <div className="flex items-center gap-2">
 <h3 className="font-bold text-foreground text-sm">{olt.nome}</h3>
 <span
 ...
 Modelo: <strong className="text-muted-foreground">{olt.modelo}</strong> • Protocolo: <strong className="text-muted-foreground">{olt.protocolo}</strong> (Porta {olt.porta})
 </p>
 </div>
 ...
 <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs bg-background/60 p-2.5 rounded-lg border border-border/80 mb-3">
 <div>
 <span className="text-[10px] text-muted-foreground block">Endereço IP</span>
 <span className="font-mono text-muted-foreground">{olt.ip}</span>
 </div>
 <div>
 <span className="text-[10px] text-muted-foreground block">POP / Localização</span>
 <span className="text-muted-foreground truncate block" title={olt.pop}>{olt.pop || 'POP Central'}</span>
 </div>
 <div>
 <span className="text-[10px] text-muted-foreground block">CPU / Temp</span>
 <span className="text-muted-foreground">{olt.cpu_usage ?? 20}% / {olt.temperatura ?? 40}°C</span>
 </div>
 <div>
 <span className="text-[10px] text-muted-foreground block">ONUs Ativas</span>
 ...
 disabled={isTesting}
 className="flex items-center gap-1 px-2.5 py-1 rounded bg-muted hover:bg-accent text-card-foreground text-xs font-medium border border-border transition-colors disabled:opacity-50"
 >
 ...
 {activeOlt && (
 <div className="bg-card/90 rounded-xl border border-border p-5 space-y-6">
 ...
 Total de Portas PON: <strong className="text-foreground">{pons.length}</strong>
 </span>
 </div>
 ...
 <div
 key={slot.id}
 className="p-3 rounded-lg bg-background/60 border border-border text-xs flex items-center justify-between"
 >
 <div>
 ...
 <span className={pon.onus_offline > 0 ? 'text-rose-400 font-medium' : 'text-muted-foreground'}>
 {pon.onus_offline}
 </span>
 </td>
