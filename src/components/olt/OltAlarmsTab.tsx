  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 bg-slate-900/40 p-3 rounded-xl border border-slate-800 text-xs">
        <div className="flex items-center gap-2">
          <span className="text-slate-400">Filtrar por OLT:</span>
          <select
            value={selectedOltId}
            onChange={(e) => setSelectedOltId(e.target.value)}
            className="px-2.5 py-1.5 rounded-lg bg-slate-950 border border-slate-800 text-slate-200 text-xs focus:outline-none focus:border-cyan-500"
          >
      ...
      <div className="space-y-3">
        {filteredAlarms.length === 0 ? (
          <div className="py-16 text-center text-slate-500 bg-slate-900/60 rounded-xl border border-slate-800">
            <CheckCircle2 size={32} className="mx-auto mb-2 text-emerald-500/60" />
            <p className="text-sm font-semibold text-slate-400">Nenhum alarme óptico pendente!</p>
            <p className="text-xs text-slate-500 mt-1">Todos os enlaces e portas PON operando dentro da margem ideal.</p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                  {!alarm.acknowledged && (
                    <button
                      ...
                      disabled={isBusy}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold border border-slate-700 transition-colors disabled:opacity-50"
                    >
