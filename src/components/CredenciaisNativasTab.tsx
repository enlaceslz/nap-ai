...
export default function CredenciaisNativasTab({
 ...
 className="p-1.5 hover:bg-accent rounded-md transition-colors text-muted-foreground hover:text-foreground shrink-0"
 title="Copiar"
 >
 ...
 <div>
 <h2 className="text-xl font-bold text-foreground font-outfit">Integrações & Endpoints Oficiais</h2>
 <p className="text-sm text-muted-foreground mt-1">
 O DJD foi construído com arquitetura <strong>Nativa e Injetada</strong>. Todas as senhas e credenciais já são carregadas via arquivo <code>.env</code> a nível de SO (Debian 12). Siga os guias abaixo para apontar os sistemas externos (CPEs, Meta, SGP, Telegram) para os conectores do DJD.
 </p>
 </div>
 ...
 <span className="text-[10px] uppercase font-bold text-muted-foreground block mb-1">ACS URL (CWMP)</span>
 <div className="flex items-center justify-between gap-2">
 <code className="text-xs text-emerald-400 break-all">http://&lt;IP_DO_DJD&gt;:7547</code>
 <CopyButton text="http://<IP_DO_DJD>:7547" id="acs-url" />
 </div>
 </div>
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block mb-1">ACS Auth (Opcional)</span>
 ...
 <div className="space-y-3">
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block mb-1">Registro SIP (Softphone Físico)</span>
 <ul className="text-xs text-muted-foreground space-y-1">
 <li><strong>Host/Domínio:</strong> &lt;IP_DO_DJD&gt;</li>
 ...
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block mb-1">WebRTC (PWA Webphone)</span>
 <p className="text-xs text-muted-foreground">
 O console do operador utiliza a porta segura <strong>WSS 8089</strong>. Necessário certificado SSL válido no DJD para funcionar o microfone no navegador.
 </p>
 </div>
 </div>
 </div>
 ...
 <p className="text-xs text-muted-foreground mb-4">
 O DJD consome a API do SGP para gerar faturas PIX e checar bloqueios. 
 </p>
 <div className="space-y-3">
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block mb-1">Como Habilitar</span>
 <ul className="text-xs text-muted-foreground space-y-2 list-decimal pl-4">
 ...
 <li>Insira a URL e o Token no arquivo <code>.env</code> do servidor DJD.</li>
 </ul>
 </div>
 </div>
 </div>
 ...
 <div className="space-y-3">
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block mb-1">URL do Webhook (No painel da Meta)</span>
 <div className="flex items-center justify-between gap-2">
 ...
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block mb-1">Token de Verificação (Verify Token)</span>
 ...
 <div className="space-y-3">
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block mb-1">Passo a Passo (Intuitivo)</span>
 <ul className="text-xs text-muted-foreground space-y-2 list-decimal pl-4">
 ...
 <li>Cole esse Token no arquivo <code>.env</code> na variável <code>TELEGRAM_BOT_TOKEN</code> e reinicie o DJD.</li>
 <li>O DJD configurará o webhook automaticamente no servidor!</li>
 ...
 <p className="text-xs text-muted-foreground mb-4">
 O DJD atua como interface inteligente consumindo os IPs do Source of Truth corporativo.
 </p>
 <div className="space-y-3">
 <div className="bg-background p-3 rounded-xl border border-border">
 <span className="text-[10px] uppercase font-bold text-muted-foreground block mb-1">Configuração (Intuitiva)</span>
 <ul className="text-xs text-muted-foreground space-y-2 list-decimal pl-4">
 ...
 <li>Caso deixe em branco, o DJD utilizará um fallback interno no PostgreSQL para reter alocações de emergência.</li>
 </ul>
 </div>
 </div>
 </div>
