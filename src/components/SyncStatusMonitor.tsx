...
export default function SyncStatusMonitor({ variant = 'topbar', className = '' }: SyncStatusMonitorProps) {
 ...
 className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-card/80 hover:bg-card border border-border hover:border-border transition-all text-xs font-medium text-muted-foreground active:scale-95"
 ...
 <span className="text-[11px] font-bold text-muted-foreground">SGP</span>
 ...
 <span className="text-[11px] font-bold text-muted-foreground">ACS</span>
 ...
 {isOpen && (
 <div className="absolute right-0 mt-2 w-96 rounded-2xl bg-card border border-border shadow-2xl p-4 z-50 text-card-foreground animate-in fade-in zoom-in-95 duration-150">
 {/* Header do Popover */}
 ...
 className="p-1.5 rounded-lg bg-white/5 hover:bg-accent text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1 text-[11px] font-bold disabled:opacity-50"
 ...
 {/* Painel do SGP */}
 <div className="mt-3.5 p-3 rounded-xl bg-background border border-border space-y-2">
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-2">
 <Server size={15} className="text-blue-400" />
 <div>
 <span className="text-xs font-bold text-foreground block leading-tight">SGP (ERP Billing)</span>
 <span className="text-[10px] text-muted-foreground font-mono block">{data?.sgp?.protocolo || 'REST API'}</span>
 </div>
 </div>
 ...
 <div>
 <span className="text-muted-foreground text-[10px] block">Latência</span>
 <span className={`font-mono font-bold ${getLatencyColor(data?.sgp?.latencia_ms)}`}>
 {data?.sgp?.latencia_ms} ms
 </span>
 </div>
 <div>
 <span className="text-muted-foreground text-[10px] block">Clientes Espelhados</span>
 <span className="font-mono font-bold text-card-foreground">
 {data?.sgp?.clientes_sincronizados} ativos
 </span>
 </div>
 <div className="col-span-2">
 <span className="text-muted-foreground text-[10px] block">Endpoint</span>
 ...
 <div>
 <span className="text-xs font-bold text-foreground block leading-tight">GenieACS (TR-069)</span>
 <span className="text-[10px] text-muted-foreground font-mono block">{acsMonitor.protocolo || 'CWMP / NBI'}</span>
 </div>
 </div>
 ...
 <div>
 <span className="text-muted-foreground text-[10px] block">CPEs Online</span>
 <span className="font-mono font-bold text-emerald-400">
 {acsOnlineCount} / {acsTotalCount}
 </span>
 </div>
 <div>
 <span className="text-muted-foreground text-[10px] block">Atenuação Óptica</span>
 <span className={`font-mono font-bold ${acsAlarms ? 'text-amber-400' : 'text-muted-foreground'}`}>
 {acsAlarms ? `${acsAlarms} alerta` : 'Normal'}
 </span>
 </div>
 <div className="col-span-3">
 <span className="text-muted-foreground text-[10px] block">NBI Endpoint Configurado</span>
 <span className="font-mono text-[10px] text-muted-foreground truncate block">
 {acsEndpoint}
 </span>
 </div>
 ...
 <span className="text-[9px] text-muted-foreground uppercase font-bold block mb-1">Pings Recentes (GenieACS NBI)</span>
 <div className="flex items-center gap-1 overflow-x-auto">
 {acsMonitor.history.slice(-6).map((h, idx) => (
 <span 
 key={idx} 
 className={`text-[9px] font-mono px-1.5 py-0.5 rounded bg-white/5 ${getLatencyColor(h.latency)} border border-border`}
 ...
 <span>Último sync: <strong className="text-muted-foreground">{getRelativeTime(data?.ultima_sincronizacao)}</strong></span>
 </div>
 ...
 <div>
 <div className="flex items-center gap-2">
 <h3 className="font-bold text-foreground text-base font-outfit">Status de Sincronização & Telecom</h3>
 ...
 {/* Card SGP */}
 <div className="p-4 rounded-xl bg-background border border-border hover:border-border transition-colors space-y-3">
 <div className="flex items-center justify-between">
 ...
 <div>
 <h4 className="font-bold text-sm text-foreground">SGP ERP Telecom</h4>
 <span className="text-[10px] text-muted-foreground font-mono">Faturamento, Clientes & Contratos</span>
 </div>
 </div>
 {getStatusBadge(data?.sgp?.status)}
 </div>

 <div className="grid grid-cols-3 gap-2 p-2.5 rounded-lg bg-white/5 border border-border text-xs">
 <div>
 <span className="text-[10px] text-muted-foreground uppercase font-bold block">Latência</span>
 <span className={`font-mono font-bold text-sm ${getLatencyColor(data?.sgp?.latencia_ms)}`}>
 {data?.sgp?.latencia_ms || '--'} ms
 </span>
 </div>
 <div>
 <span className="text-[10px] text-muted-foreground uppercase font-bold block">Clientes Sync</span>
 <span className="font-mono font-bold text-sm text-card-foreground">
 {data?.sgp?.clientes_sincronizados || 0}
 </span>
 </div>
 <div>
 <span className="text-[10px] text-muted-foreground uppercase font-bold block">Modo</span>
 <span className="font-mono font-bold text-sm text-card-foreground capitalize">
 ...
 <div>
 <div className="flex items-center gap-2">
 <h4 className="font-bold text-sm text-foreground">GenieACS (TR-069)</h4>
 {acsMonitor.validating && (
 ...
 <span className="text-[10px] text-muted-foreground font-mono">Telemetria Óptica & Wi-Fi CPE</span>
 </div>
 </div>
 {getStatusBadge(acsStatus)}
 </div>
 ...
 <div>
 <span className="text-[10px] text-muted-foreground uppercase font-bold block">ONUs Online</span>
 <span className="font-mono font-bold text-sm text-emerald-400">
 {acsOnlineCount} / {acsTotalCount}
 </span>
 </div>
 <div>
 <span className="text-[10px] text-muted-foreground uppercase font-bold block">Atenuação</span>
 <span className={`font-mono font-bold text-sm ${acsAlarms ? 'text-amber-400' : 'text-muted-foreground'}`}>
 {acsAlarms ? `${acsAlarms} Alerta` : 'Normal'}
 </span>
 </div>
 </div>
 ...
 <span className="text-[9px] text-muted-foreground uppercase font-bold">Amostras de Latência (15s):</span>
 <div className="flex items-center gap-1">
 ...
 <span>Última sincronização completa: <strong className="text-card-foreground">{getRelativeTime(data?.ultima_sincronizacao)}</strong></span>
 </div>
 <div className="flex items-center gap-3">
