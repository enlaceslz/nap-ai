  ...
  public async connect(): Promise<{ success: boolean; message: string; latency_ms: number }> {
    ...
    return new Promise((resolve) => {
      ...
      socket.once('connect', () => {
        ...
        resolve({
          success: true,
          message: `Conexão TCP/SSH estabelecida com sucesso na OLT ZTE ${this.olt.modelo} (${this.olt.ip}:${port})`,
          latency_ms: latency
        });
      });
      socket.once('timeout', () => {
        ...
        resolve({
          success: false,
          message: `Timeout ao conectar na OLT ZTE (${this.olt.ip}:${port})`,
          latency_ms: 2500
        });
      });
      socket.once('error', (err) => {
        ...
        resolve({
          success: false,
          message: `Falha ao conectar na OLT ZTE (${this.olt.ip}:${port}): ${err.message}`,
