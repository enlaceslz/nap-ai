...
if (!text.includes('Score:')) {
  ...
  text = text.replace(
    /\{viabilityResult\.viable \? \([\s\S]*?\) : \([\s\S]*?\)\}/,
    newViabilityUI
  );

  // Update Polyline rendering for viability drop line
  text = text.replace(
  ...
  text = text.replace(
    /<div className="flex justify-between items-start mb-2">[\s\S]*?<\/div>[\s\S]*?<\/div>/,
    newImpactUI
  );

  fs.writeFileSync('src/pages/GisDashboard.tsx', text, 'utf8');
  console.log('GisDashboard UI patched for Part 03');
} else {
