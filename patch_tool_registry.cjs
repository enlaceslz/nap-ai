...
if (!code.includes("import { ErpFactory }")) {
  ...
  // Update sgp_gerar_pix implementation
  code = code.replace(
    /execute: async \(\{ cliente_cpf \}\) => \{[\s\S]*?const resposta = \[\[\[\[\n/g,
    ...
    if (cliente) {
      ...
      resposta = \`Infelizmente não consegui localizar um cliente com o CPF/CNPJ \${cpf} no nosso sistema.\`;
    }
...
`
  ...
  // Fix the return structure in the replaced block.
  // Wait, the regex replacement might break if I don't match it perfectly. Let's do it safer.
}
